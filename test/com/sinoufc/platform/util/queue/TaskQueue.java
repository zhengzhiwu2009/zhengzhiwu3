...
import com.sinoufc.platform.util.queue.Task.OnSystemFinishListen;
import com.sinoufc.platform.util.queue.Task.TaskStatus;
...
//			System.out.println( "执行队列中移除任务taskid=" + t.taskID);
			// 通知执行后续未处理的任务
			System.out.println("Queue--队列正在执行任务数" + tasks_running.size() + "/等待"+tasks_wait.size()+ "/上限" + ThreadMaxNum);
