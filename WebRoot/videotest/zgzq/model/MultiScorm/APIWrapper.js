...
/*******************************************************************************
**
** Function doLMSSetSCOID(scoID)
** Inputs:  scoId -string 当前sco的id值，替代原有点击menu时，在sequencingEngine.jsp设置scoId的操作
** Return:  CMIBoolean true if successful
**          CMIBoolean false if failed.
**
** Description:
** Wraps the call to the LMS LMSSetSCOID function
**
*******************************************************************************/
function doLMSSetSCOID(scoID)
{
   var api = getAPIHandle();
   if (whatyExtAPIHandle == null)
   {
      trace("Unable to locate the LMS's whatyExtAPIHandle Implementation.\ndoLMSSetSCOID was not successful.");
      return;
   }
   else
   {
      var result = whatyExtAPIHandle.setSCOID(scoID);
      if (result.toString()!="true")
      {
         var err = ErrorHandler();
      }
...
function findAPI(win)
{
   while ((win.API == null|win.API==undefined) && (win.parent != null) && (win.parent != win))
   {
      ...
      if (findAPITries > 7) 
      {
         trace("Error finding API -- too deeply nested.");
         return null;
      }
      win = win.parent;
   }
   if(win.whatyExtAPI)	whatyExtAPIHandle=win.whatyExtAPI;
   return win.API;
}
/*******************************************************************************
**
...
{
	var thisWin = window;
	var theAPI = findAPI(thisWin);
	var tryTimes=0;
	while ((theAPI == null|theAPI==undefined) && thisWin.top.opener)
	{
		tryTimes++;
		if(tryTimes>7)
		{
			trace("all opener searched! no API found!");
			return null;
		}
		thisWin = thisWin.top.opener;
		theAPI = findAPI(thisWin);
	}
	if (theAPI == null)
	{
