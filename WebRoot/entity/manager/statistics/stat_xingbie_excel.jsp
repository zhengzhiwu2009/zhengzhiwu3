...
<%	response.setHeader("Content-Disposition", "attachment;filename=stat_xingbie.xls"); %>
...
<%
	...
	if(us.getRoleId().equals("2") || us.getRoleId().equals("402880a92137be1c012137db62100006"))  //表示为一级主管和一级辅导员
	{
		...
		sql = "select a.total_female, b.total_male "+
              ...
              "from pe_bzz_batch b, (select s.gender,s.fk_batch_id,s.fk_enterprise_id from pe_bzz_student s,sso_user u where u.id=s.fk_sso_user_id and u.flag_isvalid='2' and s.flag_rank_state='402880f827f5b99b0127f5bdadc70006') s, pe_enterprise e "+
              "where s.gender = '女' "+
              "and s.fk_batch_id = b.id and e.id = s.fk_enterprise_id "+sql_con+" ) a, "+
              "(select count(si.gender) as total_male "+
              " from pe_bzz_batch b, (select s.gender,s.fk_batch_id,s.fk_enterprise_id from pe_bzz_student s,sso_user u where u.id=s.fk_sso_user_id and u.flag_isvalid='2' and s.flag_rank_state='402880f827f5b99b0127f5bdadc70006') si, pe_enterprise e "+
              "where si.gender = '男' " +
       ...
       // System.out.println(sql); 
		...
		count = Integer.parseInt(total_female) + Integer.parseInt(total_male);
		db.close(rs);
%>
