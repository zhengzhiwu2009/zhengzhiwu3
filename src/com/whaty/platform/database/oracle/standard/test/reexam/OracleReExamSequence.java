...
public class OracleReExamSequence extends ReExamSequence {
	public OracleReExamSequence() {
	}

	public OracleReExamSequence(String aid) {
		dbpool db = new dbpool();
		String sql = "";
		MyResultSet rs = null;
		sql = "select title,to_char(startdate,'yyyy-mm-dd HH24:MI:SS') as s_date,to_char(enddate,'yyyy-mm-dd HH24:MI:SS') as e_date,note,testbatch_id,basicsequence_id from expendtest_examsequence_info where id='"
				+ aid + "'";
		try {
	...
	public int add() throws PlatformException {
		dbpool db = new dbpool();
		String sql = "";
		sql = "insert into expendtest_examsequence_info (id,title,startdate,enddate,note,testbatch_id,basicsequence_id) values (to_char(s_test_examsequence_info_id.nextval),'"
				+ this.getTitle()
				+ "',to_date('"
				+ this.getStartDate()
	...
	public int addWithBatch() throws PlatformException {
		...
		String sql1 = "select name from expendtest_exambatch_info where id="+thisBatchId+"";
		...
		String sql = "insert into expendtest_examsequence_info (id,title,startdate,enddate,note,testbatch_id,basicsequence_id) values (to_char(s_test_examsequence_info_id.nextval),'"
	...
	public int update() throws PlatformException {
		...
		String sql = "";
		String thisBatchId = this.getBatchId();
		sql1 = "select name from expendtest_exambatch_info where id="+thisBatchId+"";
		rss = db.executeQuery(sql1);
		String exambatch_name = "";
		try {
			while(rss!=null&&rss.next()){
		...
		sql = "update expendtest_examsequence_info set title='" + exambatch_name+this.getTitle()
	...
	public int delete() throws PlatformException {
		...
		String sql1 = "select t.id from expendtest_examcourse_info t ,expendtest_examsequence_info t2 " +
	...
	if(ii>0){
		throw new PlatformException(this.getTitle()+ "Ѱſγ̣");
		 ...
		 String sql = "delete from expendtest_examsequence_info where id='" + this.getId()
				+ "'";
		int i = db.executeUpdate(sql);
