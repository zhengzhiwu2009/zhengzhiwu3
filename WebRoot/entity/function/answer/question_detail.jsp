					...
					<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
							...
							<td class="td"><%=answer.getReuserName()%>
								<%
									...
									if (teacher != null) {
										...
										if (answertea_id.equals(teacher_id) && us.getRoleId().equals("3")) {
								%>
								<a href="answer_edit.jsp?answer_id=<%=answer.getId()%>&question_id=<%=question.getId()%>&pageInt=<%=pageInt%>&title=<%=title%>&name=<%=name%>" class="tj">[修改答案]</a>
								<a href="javascript:cfmdel('answer_delexe.jsp?answer_id=<%=answer.getId()%>&question_id=<%=question.getId()%>&pageInt=<%=pageInt%>&title=<%=title%>&name=<%=name%>')" class="tj">[删除此答案]</a>
								<%
											}
										}
									}
						...
						<tr>
							<td colspan="6" class="td">
								<%
									if (teacher != null && "teacher".equals(userType) && us.getRoleId().equals("3")) {
								%>
								&nbsp;&nbsp;
								<a href="answer_add.jsp?id=<%=question_id%>&pageInt=<%=pageInt%>&title=<%=title%>&name=<%=name%>&a_title=<%=question.getTitle()%>" class="tj">[回答]</a>&nbsp;&nbsp;
								<%
									}
								...
								<%
										...
										if ((student != null && question.getSubmituserId().equals(student.getId()) && !question.isReStatus()) || (teacher != null && "teacher".equals(userType) && us.getRoleId().equals("3"))) {
								%>
								&nbsp;&nbsp;
								<a href="javascript:cfmdel('question_delexe.jsp?id=<%=question_id%>&pageInt=<%=pageInt%>&title=<%=title%>&name=<%=name%>')" class="tj">[删除问题(包括答案)]</a>&nbsp;&nbsp;
								<%
									} else if (teacher != null && "teacher".equals(userType) && us.getRoleId().equals("3")) {
								%>
								&nbsp;&nbsp;
								<a href="javascript:cfmdel('question_delexe.jsp?id=<%=question_id%>&pageInt=<%=pageInt%>&title=<%=title%>&name=<%=name%>')" class="tj">[删除问题(包括答案)]</a>&nbsp;&nbsp;
								<%
								...
								%>
								&nbsp;&nbsp;
								<a href="faq_add.jsp?id=<%=question_id%>" class="tj">[收入常见问题库]</a>&nbsp;&nbsp;
								<%
									}
								%>
								&nbsp;&nbsp;
