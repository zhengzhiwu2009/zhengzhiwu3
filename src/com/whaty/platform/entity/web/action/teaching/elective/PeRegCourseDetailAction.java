...
public class PeRegCourseDetailAction extends MyBaseAction<PeBzzTchCourse> {
	...
	public void initGrid() {
		...
		this.getGridConfig().addColumn(this.getText("课程时长"), "course_len", false, false, true, "TextField");
...
//		this.getGridConfig().addColumn(this.getText("考试结果"), "learningCourse", false, false, true, "TextField");
		...
		this.getGridConfig().addRenderScript(this.getText("查看学习详情"),
				"{return '<a href=\"/entity/teaching/searchAnyStudent_courseInfo.action?allid='+${value}+'&id=" + this.getId() + "&type=" + type + "\" target=\"_blank\">查看学习详情</a>';}", "id");

		this.getGridConfig().addMenuScript(this.getText("返回"), "{location.href='entity/basic/peBzzCourseRegSearch.action'};");
	...
	public void setServletPath() {
		// TODO Auto-generated method stub
		this.servletPath = "/entity/teaching/peRegCourseDetail";
	}
	...
	public Page list() {
		...
		try {
			...
			sqlBuffer.append("select pbs.id || '-' || pbtc.id || '-' || pbto.id as id,pbtc.code,pbtc.name,course_len,tcs.get_date,tcs.complete_date, ");
			...
			sqlBuffer.append("       case when ec1.code = '1' and pbtse.ispass = '1' then '已通过考试' ");
			...
			sqlBuffer.append("  join pe_bzz_student pbs on su.id = pbs.fk_sso_user_id ");
			...
			sqlBuffer.append("  join enum_const ec4 on pbtc.flag_coursearea = ec4.id ");
			sqlBuffer.append(" where ec2.code = '1' and ec3.code = '0' and ec4.code in ('1','2','5') ");
			sqlBuffer.append(" and (pbb.id='40288a7b394d676d01394dad824c003b' or pbto.fk_batch_id is null) and pbs.id = '" + this.getId() + "'");
	...
	public String learnCourseDetail(){
		...
		sqlBuffer.append(" where (pbb.id = '40288a7b394d676d01394dad824c003b' or pbto.fk_batch_id is null) ");
		sqlBuffer.append(" and pbtc.id='" + this.getCourseId() + "'");
		try {
