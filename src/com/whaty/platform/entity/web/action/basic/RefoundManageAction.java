...
public class RefoundManageAction extends MyBaseAction<PeBzzRefundInfo> {
	...
	public void initGrid() {
		...
		this.getGridConfig().setTitle(this.getText("退费初审"));
		...
		//this.getGridConfig().addColumn(this.getText("支付方式"), "peBzzOrderInfo.enumConstByFlagPaymentMethod.name");
		...
		this.getGridConfig().addRenderFunction("查看详情", "<a href='/entity/basic/refoundManage_viewDetail.action?bean.id=${value}' >查看</a>",
				"id");

		if (capabilitySet.contains(this.servletPath + "_firstAudit.action")) {// 按钮权限判断
			this.getGridConfig().addMenuFunction(this.getText("初审确认"), "/entity/basic/refoundManage_toRefund.action", false, true);
			...
			this.getGridConfig().addMenuScript(
					...
					"{" + "var m=grid.getSelections();" + "    var ids='';" + "	if(m.length>0){"
							...
							+ "       formPanel.form.submit({" + "        url:'/entity/basic/refoundManage_refuseRefund.action',"
	...
	public void setServletPath() {
		// TODO Auto-generated method stub
		this.servletPath = "/entity/basic/refoundManage";
	}
	...
	public Page list() {
		...
		try {
			...
			sb.append("        EC2.NAME  || '' || CASE WHEN pboi.payment_method ='10' THEN '-个人网银' WHEN pboi.payment_method ='14' THEN '-企业网银' ELSE '' END   EC2NAME, ");
	 ...
	 */
	/*
	 * @Override public DetachedCriteria initDetachedCriteria() { // TODO
	 ...
	 * "enumConstByFlagOrderType"); dc.createAlias("enumConstByFlagRefundState",
	 * "enumConstByFlagRefundState"); try { // 处理时间的查询参数 ActionContext context =
	 * ActionContext.getContext(); if (context.getParameters() != null) { Map
	 * params = this.getParamsMap(); if (params != null) { if
	 * (params.get("search__applyStartDate") != null) { String[] startDate =
	 * (String[]) params.get("search__applyStartDate"); String tempDate =
		...
		if (this.getIds() != null && this.getIds().length() > 0) {
			...
			try {
				...
				if (peBzzRefundInfo.getEnumConstByFlagRefundState().getCode().equals("1")
						|| peBzzRefundInfo.getEnumConstByFlagRefundState().getCode().equals("4")) {
					map.clear();
					map.put("success", "false");
				...
				// }
				// 修改订单未初审通过状态
				EnumConst enumConstByFlagRefundState = this.getEnumConstService().getByNamespaceCode("FlagRefundState", "4");// 初审通过状态
	...
	public String refuseRefund() {
		...
		if (this.getIds() != null && this.getIds().length() > 0) {
			...
			try {
				...
				if (!peBzzRefundInfo.getEnumConstByFlagRefundState().getCode().equals("0")) {
					map.clear();
					map.put("success", "false");
					map.put("info", "已经审核过，不能操作");
					this.setJsonString(JsonUtil.toJSONString(map));
					return this.json();
				}
