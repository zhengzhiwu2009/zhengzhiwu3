...
public class WhatyUploaderServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		if (debug)
			System.out.println("---- WhatyUploaderServlet initialization completed ----\r\n");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (enabled) {
			try {
				...
				// 设定使用的parser,cos为.COSPARSER,structs为STRUTSPARSER,Commons-FileUpload
				// parsers 为 CFUPARSER
				...
				if ((files != null) && (files.length != 0)) {
					...
					// Lee 获取文件名(去掉后缀名)
					String file_name = fileName.substring(0, fileName.lastIndexOf("." + ext));
					...
					// Lee 原上传后将文件名改为日期 现加上原文件名前缀
					// s_filename = YY + MM + DD + HH + NN + SS + "." + ext;
					s_filename = file_name + "_" + YY + MM + DD + HH + NN + SS + "." + ext;
