...
var scoManager = 	{
						...
						creatSSObj:function(obj)
						{
							...
							{   
								returnObj.duration=		obj.duration;
								returnObj.status=		obj.status;
								returnObj.score=		obj.score;
								returnObj.effects=		obj.duration+"|"+obj.score;
								if(obj.href!="#")
									returnObj.links=	obj.href;
							}
							if(typeof(obj.item) != "undefined" && obj.item.length>0)
							{
								var tmpstr=this.calTotalDuration(obj);
								returnObj.duration=	tmpstr.substring(0,tmpstr.indexOf("."));	
								returnObj.score = "";//this.calTotalScore(obj);并没有使用
								returnObj.status = this.calTotalStatus(obj);
								returnObj.effects = "";//returnObj.duration + "|" + returnObj.score;并没有使用
							...
							for(var i=0;i<obj.item.length;i++)
							{
								...
								returnObj.children.push(this.creatSSObj(obj.item[i]));
						...
						{
							var totalScore = 0;
							if(obj.score!=null&&obj.score!='' && obj.score!="null")
							{
								totalScore =	this.combScore(totalScore,obj.score);
							}
							//
							for(var i=0;i<obj.item.length;i++)
							{
								totalScore = 	this.combScore(totalScore,this.calTotalScore(obj.item[i]));
							}
							if(obj.item.length != 0)
								return parseInt(totalScore / obj.item.length);
							else
								return totalScore;
						},
						//设置完成状态
						...
						{
							var totalNum = 0;
							if( obj.status!="null" && obj.item.length == 0)
							{
								return ++totalNum;
							}
							for(var i=0;i<obj.item.length;i++)
							{
								totalNum += this.getTotalScoNum(obj.item[i]);
							}
							return totalNum;
						},
						...
						getCompletedScoNum:function(obj)
						{
							var totalNum = 0;
							if(obj.status!='' 
								&& obj.status!="null" && obj.item.length == 0)
							{
							...
							for(var i=0;i<obj.item.length;i++)
							{
								totalNum += this.getCompletedScoNum(obj.item[i]);
							}
							return totalNum;
						},
						calTotalDuration:function(obj)
						{
							...
							if(obj.duration!=null&&obj.duration!='' && obj.duration!="null")
							{
								totalDuration =		this.combTime(totalDuration,obj.duration);
							}
							//
							for(var i=0;i<obj.item.length;i++)
							{
								//var pretotal = totalDuration;
								//var tmpstr=this.calTotalDuration(obj.item[i]);
								totalDuration = 	this.combTime(totalDuration,this.calTotalDuration(obj.item[i]));
