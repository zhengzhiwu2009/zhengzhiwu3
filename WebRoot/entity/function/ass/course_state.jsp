...
<%
	...
	String sql = " SELECT DISTINCT  PBTC.CODE CODE, PBTC.NAME NAME, EC.NAME EC1NAME, PBTC.TIME, " + " TO_CHAR(TCS.GET_DATE, 'yyyy-MM-dd hh24:mi:ss') START_TIME, "
			...
			//+ " WHEN TCS.LEARN_STATUS = 'COMPLETED' AND PBTC.FLAG_IS_EXAM = '40288acf3ae01103013ae012940b0001' AND PBTUE.SCORE_EXAM < 80 THEN 'FAILED' "
			...
			+ " INNER JOIN PR_BZZ_TCH_USER_ELECTIVE PBTUE ON PBTO.ID = PBTUE.FK_TCH_OPENCOURSE_ID " + " INNER JOIN SSO_USER SU ON PBTUE.FK_USER_ID = SU.ID "
			...
			+ " LEFT JOIN TRAINING_COURSE_STUDENT TCS ON PBTUE.FK_TRAINING_ID = TCS.ID " + " WHERE SU.ID = '" + uid + "' AND PBTC.ID = '" + courseId + "' ";
	...
	sql = " SELECT to_char(START_TIME,'YYYY-MM-DD HH24:MI:SS') START_TIME, to_char(END_TIME,'YYYY-MM-DD HH24:MI:SS') END_TIME, TOTAL_TIME FROM ( "
			+ " SELECT A.FK_ORDER_ID, A.START_TIME, A.END_TIME, A.TOTAL_TIME FROM  "
			+ " TRAINING_COURSE_STUDENT_HIS A, PR_BZZ_TCH_USER_ELECTIVE B, SSO_USER D, PR_BZZ_TCH_OPENCOURSE E, PE_BZZ_TCH_COURSE F "
			+ " WHERE A.FK_STU_ID = B.FK_USER_ID AND A.COURSE_ID = B.FK_TCH_OPENCOURSE_ID AND B.FK_USER_ID = D.ID "
			+ " AND B.FK_TCH_OPENCOURSE_ID = E.ID AND E.FK_COURSE_ID = F.ID AND D.ID = '"
			+ uid
			+ "' AND F.ID = '"
			+ courseId
			+ "') A "
	...
	if ("true".equalsIgnoreCase(exam_y_n)) {
		sql = " SELECT DISTINCT DECODE(TEST_OPEN_DATE,NULL,'-',TEST_OPEN_DATE) OPEN_TIME, TO_CHAR(G.TEST_DATE, 'yyyy-MM-dd hh24:mi:ss') SUBMIT_TIME, A.EXAMTIMES_ALLOW, A.PASSROLE, G.SCORE SCORE_EXAM, G.SCORE-A.PASSROLE FENSHUCHA ,D.ISPASS "
				+ " FROM PE_BZZ_TCH_COURSE A INNER JOIN PR_BZZ_TCH_OPENCOURSE B ON A.ID = B.FK_COURSE_ID AND A.ID = '"
				+ courseId
				+ "' "
				+ " INNER JOIN TEST_TESTPAPER_INFO C ON A.ID = C.GROUP_ID INNER JOIN PR_BZZ_TCH_STU_ELECTIVE D ON B.ID = D.FK_TCH_OPENCOURSE_ID "
				+ " INNER JOIN PE_BZZ_STUDENT E ON D.FK_STU_ID = E.ID AND E.FK_SSO_USER_ID = '"
				+ uid
				+ "' INNER JOIN ENUM_CONST F "
				+ " ON A.FLAG_IS_EXAM = F.ID INNER JOIN TEST_TESTPAPER_HISTORY G ON C.ID = G.TESTPAPER_ID AND E.FK_SSO_USER_ID = G.T_USER_ID ORDER BY SUBMIT_TIME ASC";
		MyResultSet mrsTestList = null;
		mrsTestList = db.executeQuery(sql);

		if (null == mrsTestList || !mrsTestList.next()) {
			db.close(mrsTestList);
			sql = " SELECT DISTINCT TTH.TEST_OPEN_DATE OPEN_TIME, TO_CHAR(TTH.TEST_DATE, 'yyyy-MM-dd hh24:mi:ss') SUBMIT_TIME, "
					+ " COURSE_MEGRE.EXAMTIMES_ALLOW EXAMTIMES_ALLOW, COURSE_MEGRE.PASSROLE PASSROLE, TTH.SCORE SCORE_EXAM, "
					+ " CASE WHEN TO_NUMBER(TTH.SCORE) >= TO_NUMBER(COURSE_MEGRE.PASSROLE) THEN '1' ELSE '0' END AS ISPASS, "
					+ " TTH.SCORE - COURSE_MEGRE.PASSROLE FENSHUCHA FROM TEST_TESTPAPER_HISTORY TTH JOIN TEST_TESTPAPER_INFO TTI "
					+ " ON TTI.ID = TTH.TESTPAPER_ID JOIN SSO_USER PBS ON PBS.ID = TTH.T_USER_ID "
					+ " JOIN PR_BZZ_TCH_OPENCOURSE PBTO ON PBTO.FK_COURSE_ID = TTI.GROUP_ID JOIN (SELECT PBTC.ID, "
					+ " PBTO.ID PBTO_ID, PBTSE.FK_USER_ID, PBTC.EXAMTIMES_ALLOW, PBTC.PASSROLE "
					+ " FROM PE_BZZ_TCH_COURSE PBTC JOIN PR_BZZ_TCH_OPENCOURSE PBTO ON PBTO.FK_COURSE_ID = PBTC.ID "
					+ " JOIN PR_BZZ_TCH_USER_ELECTIVE PBTSE ON PBTSE.FK_TCH_OPENCOURSE_ID = PBTO.ID  " + " WHERE PBTC.FLAG_IS_EXAM = '40288acf3ae01103013ae012940b0001' "
					+ " GROUP BY PBTC.ID, PBTO.ID, PBTSE.FK_USER_ID, PBTC.EXAMTIMES_ALLOW, PBTC.PASSROLE) COURSE_MEGRE "
					+ " ON COURSE_MEGRE.ID = TTI.GROUP_ID AND PBS.ID = COURSE_MEGRE.FK_USER_ID AND COURSE_MEGRE.PBTO_ID = PBTO.ID " + " WHERE TTI.GROUP_ID = '" + courseId + "' AND PBS.ID = '"
					+ uid + "' order by tth.test_open_date asc ";
			mrsTestList = db.executeQuery(sql);
		}
