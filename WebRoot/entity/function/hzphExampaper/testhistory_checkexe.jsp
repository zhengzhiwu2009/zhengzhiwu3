...
%>	
<%		
		String piciStudent =request.getParameter("piciStudent");
		String piciId = request.getParameter("piciId");
		String tsId = request.getParameter("tsId");
		String paperId = request.getParameter("paperId");
		String pageInt = request.getParameter("pageInt");
		String paperUserId = request.getParameter("paperUserId");
		String realUserId = paperUserId.substring(1,paperUserId.indexOf(")"));
		...
		idList.add(realUserId);
		
		
		int count = testManage.updateTestPaperHistory(tsId,paperUserId,paperId,map,piciStudent);
		
		try{
			Map scoreMap = basicScoreManage.getOnlineExamScore(realUserId,openCourse.getId());
			double passScore=80;
			double score=0;
			if(scoreMap.containsKey("stuScore")){
				score=(Double)scoreMap.get("stuScore");
			}
			if(scoreMap.containsKey("passScore")){
				passScore=(Double)scoreMap.get("passScore");
			}
			
			if(Double.valueOf(totalScore) >score ){
				String ispass="0";
				if(Double.valueOf(totalScore) >=passScore){
					ispass="1";
				}
				basicScoreManage.updateOnlineExamScore(realUserId,openCourse.getId(),totalScore,ispass);	
			}
		}
		catch(Exception e)
		{
...
<script type="text/javascript">
	alert("考试答卷保存成功!");
	location.href = "/entity/onlineexam/pePcDetail.action?id=<%=piciId %>&method=mystudent";
</script>
<%
		} else {
%>
