...
public class PrExamScoreInputUserAcion extends MyBaseAction<PeTchCourse> {
	...
	public void initGrid() {
		...
		this.getGridConfig().addMenuFunction(this.getText("修改课程登分账户为别的登分账户"), "/entity/exam/prexamscoreinputuser_gotoEdit.action", true, false);
		
		this.getGridConfig().setCapability(false, false, false,true);
		this.getGridConfig().setTitle(this.getText("课程登分账户数据信息"));
		...
		ColumnConfig c1 = new ColumnConfig(this.getText("登分账户A"),"combobox_PeExamScoreInputUser.aName");
		c1.setComboSQL("select id,name from pe_exam_score_input_user where name like '%a'");
		this.getGridConfig().addColumn(c1);
		//this.getGridConfig().addColumn(this.getText("登分账户A"), "combobox_PeExamScoreInputUser.aName");		
		this.getGridConfig().addColumn(this.getText("A密码"), "aPassword",false);
		ColumnConfig c2 = new ColumnConfig(this.getText("登分账户B"),"combobox_PeExamScoreInputUser.bName");
		c2.setComboSQL("select id,name from pe_exam_score_input_user where name like '%b'");
		this.getGridConfig().addColumn(c2);
		//this.getGridConfig().addColumn(this.getText("登分账户B"), "combobox_PeExamScoreInputUser.bName");
