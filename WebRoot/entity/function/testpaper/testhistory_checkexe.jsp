...
<%
		...
		int count = testManage.updateTestPaperHistory(tsId,paperUserId,paperId,map);
		
		try{
			String score = basicScoreManage.getElectiveScore(realUserId,openCourse.getId());
			//basicScoreManage.updateScoreBatch(null,null,null,null,null,scoreList,null, idList, null,null,openCourse.getId());
			if(Double.valueOf(totalScore) > 80 && Double.valueOf(totalScore) > Double.valueOf(score)){
				basicScoreManage.updateElectiveScore(realUserId,openCourse.getId(),totalScore);	
			}
		}
		catch(Exception e)
		{
...
<script type="text/javascript">
	alert("批改答卷保存成功!");
	location.href = "testhistory_list.jsp?pageInt=<%=pageInt%>";
</script>
<%
		} else {
%>
