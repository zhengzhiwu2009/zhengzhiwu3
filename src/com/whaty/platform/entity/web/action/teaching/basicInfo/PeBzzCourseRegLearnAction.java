...
public class PeBzzCourseRegLearnAction extends MyBaseAction<PeBzzTchCourse> {
	...
	public void initGrid() {
		...
		this.getGridConfig().setTitle("监管机构内训课程学习");
		...
		String sql1 = "select a.id ,a.name from enum_const a where a.namespace='FlagCoursearea' and a.code not in ('0','3','4')";
		...
		this
				...
				.addRenderScript(
						this.getText("课程信息"),
						"{if(record.data['enumConstByFlagCourseArea.name']=='公共区课程'){return '<a href=\"/entity/teaching/peBzzCoursePubManager_courseInfo.action?id='+${value}+'&indexFile='+record.data['indexFile']+'\"  target=\"_blank\">课程信息</a>';} else if(record.data['enumConstByFlagCourseArea.name']=='专项课程'){return '<a href=\"/entity/teaching/peBzzCourseBatManager_courseInfo.action?id='+${value}+'&indexFile='+record.data['indexFile']+'\"  target=\"_blank\">课程信息</a>';} else if(record.data['enumConstByFlagCourseArea.name']=='监管机构内训课程'){return '<a href=\"/entity/teaching/peBzzCourseRegManager_courseInfo.action?id='+${value}+'&indexFile='+record.data['indexFile']+'\"  target=\"_blank\">课程信息</a>';}}",
						"id");

		if (capabilitySet.contains("/entity/teaching/peRegCourseDetail_learnCourseDetail.action")) {
			this.getGridConfig().addRenderFunction(this.getText("学习详情"), "<center><a href=\"/entity/teaching/peRegCourseDetail_learnCourseDetail.action?courseId=${value}&type=1\" target='_blank'>学习详情</a></center>", "id");
		}
	...
	public void setServletPath() {
		this.servletPath = "/entity/teaching/peBzzCourseRegLearn";
	...
	public Page list() {
		...
		try {
			...
			sqlBuffer.append("               join training_course_student tcs on pbto.id = tcs.course_id ");
			...
			sqlBuffer.append(" where ec7.code = '1' and ec8.code = '0' and ec5.code in ('1','2','5') ");
			...
			if (this.getSort() != null && temp != null) {
				...
				/* 课程来源 */
				if (temp.equals("enumConstByFlagCourseArea")) {
					temp = "FlagCourseArea";
				}
				/* 学习状态 */
