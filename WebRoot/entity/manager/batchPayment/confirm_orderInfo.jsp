...
function tishi(id){
	...
	<body leftmargin="0" topmargin="0" class="scllbar">
		<form name="form" id="form"
			...
			<input type="hidden" id="amount1" name="amount" value="<s:property value="#session.user_session.ssoUser.amount" />" />
			<input type="hidden" id="blance" name="blance" value="<s:property value="#session.batchPrice"/>" />
			<div class="warp">
			<h1>订单确认</h1>
			<table width="890" border="0" cellspacing="0" cellpadding="0" class="datalist">
				<tr>
					<th width="164"><div align="right">总金额：</div></th>
					<td width="726"><s:property value="#session.batchPrice"/>元
					</td>
				</tr>
				<s:if test="#request.zxFlag != 'true'">
				<tr>
					<th>预付费账户剩余学时:</th>
					<td><s:property value="#request.sumAmount-#request.amount"/>学时</td>
				</tr>
				</s:if>
				<tr>
					<th><div align="right">订单备忘录：</div></th>
					<td>
						<input name="peBzzOrderInfo.cname" id="orderAlias" type="text" size="25" class="txt1" 
							maxlength="15"/>*便于您个人记住订单的标识。
					</td>
				</tr>
				<tr>
					<th><div align="right">选择支付方式：</div></th>
					<td>
						<input class="radioItem" type="radio" onchange ="tishi(this.id)" id="online"  name="payMethod" value="0"/>
						在线支付
						<s:if test="#session.Logintype == 2">
							<input class="radioItem" type="radio" onchange ="tishi(this.id)" id ="gongs" name="payMethod" value="2" <s:if test="#session.batchPrice < 10000">disabled="disabled"</s:if>/>
							公司转账(当订单金额大于或等于一万元时，方可使用)
						</s:if>
						<s:if test="#request.zxFlag != 'true'">
						<input class="radioItem" type="radio" name="payMethod"  <s:if test="#session.batchPrice * 1 /#request.amount * 1 != 20"> disabled="disabled" </s:if> value="1" />
...
--%>					</td>
		...
		<div>
			<table id="invoice" width="890" border="0" cellspacing="0" cellpadding="0" class="datalist" style="display:none;">
			    <tr>
			      <td colspan="2" bgcolor="#cff2f6" class="title" >请认真如实填写或补充完整下面的地址信息，我们将发票邮寄到你填写的地址，请务必如实填写</td>
			    </tr>
				<tr>
					<th width="164"><div align="right">付款单位（个人）：</div></th>
					<td width="726">
				...
				<tr>
					<th><div align="right">邮寄编码：</div></th>
					<td>
				...
				<tr>
					<th><div align="right">联系电话：</div></th>
					<td>
				...
				<tr>
					<th><div align="right">邮寄地址：</div></th>
					<td>
		...
		<div>	
			<br/><table id="paymentInfo" width="890" border="0" cellspacing="0" cellpadding="0" class="datalist" style="display:none;">
			    <tr>
				...
				<tr>
			        <th width="164"><div align="right">填写汇款银行账号：</div><br/></th>
			        <td width="726">
				...
				<tr>
					<th><div align="right">再次填写汇款银行账号：</div><br/></th>
					<td>
