...
public class OracleExamActivity extends ExamActivity {

	public OracleExamActivity() {
	}
	...
	public void courseAddUser(List examCourseList, List examUserList)
		...
		for (int i = 0; i < examCourseList.size(); i++) {
			examCourse = (ExamCourse) examCourseList.get(i);
			for (int j = 0; j < examUserList.size(); j++) {
				examUser = (ExamUser) examUserList.get(j);
	...
	public List getSiteTestRooms(String activeBatchId,String course_id,String siteId, Page page,
			HttpServletRequest request) throws PlatformException {
		...
		String sql = 
			...
			"        and esi.site_id = esti.id and teub.batch_id='" +activeBatchId + "' and teuc.course_id='" +course_id+"'"+
			"         )" + 
	...
	public List getSiteTestRooms1(String activeBatchId,String course_id,String siteId, Page page,
		...
		String sql = 
			...
			"  from test_examroom_info   t," + 
			"       test_examcourse_info teci," + 
			"       entity_edu_type      eet," + 
			...
			"   and ki.id = t.kaoqu_id and  egi.id = t.grade_id) order by room_id";
		ArrayList testRooms = new ArrayList();
		try {
			db = new dbpool();
	...
	public List getConnect_status(String room_id,String activeBatchId,String course_id,String siteId, Page page,
		...
		String conditions = "";
		String order = "";
			String sql = "select t.connect_status as connect_status from test_examroom_info t where id ='"+room_id+"'";
	...
	public int getRoom_no(String room_no,String room_id,String activeBatchId,String course_id,String siteId, Page page,
			HttpServletRequest request) throws PlatformException {
		...
		String conditions = "";
		String order = "";
			String sql = "select room_num from test_examroom_info  where room_no='"+room_no+"'";
		int i1=0;
		ArrayList testRooms = new ArrayList();
		try {
			db = new dbpool();
...
//				HashMap course = new HashMap();
//				course.put("connect_status", rs.getString("connect_status"));//
				i1 += Integer.parseInt(rs.getString("room_num"));
	...
	public int getReRoom_no(String room_no,String room_id,String activeBatchId,String course_id,String siteId, Page page,
			HttpServletRequest request) throws PlatformException {
		...
		String conditions = "";
		String order = "";
			String sql = "select room_num from expendtest_examroom_info  where room_no='"+room_no+"'";
		int i1=0;
		ArrayList testRooms = new ArrayList();
		try {
			db = new dbpool();
...
//				course.put("connect_status", rs.getString("connect_status"));//
	...
	public int getSiteTestRoomsNum(String activeBatchId,String course_id,String siteId, HttpServletRequest request)
		throws PlatformException {
	...
	String sql = 	"select course_id,course_name,edu_type_id,edu_type_name,major_id,major_name,kaoqu_id, kaoqu_name,shenfen_id,grade_id,grade_name," +
		"count(users) as useres" + 
		"  from (select teci.id as course_id,  teci.name as course_name, teuc.user_id as users,eet.id as edu_type_id,eet.name as edu_type_name," + 
		"    emi.id as major_id,emi.name as major_name,eki.id as kaoqu_id,eki.name as kaoqu_name,esi.shenfen_id as shenfen_id," + 
		"       egi.id as grade_id,egi.name as grade_name, esi.site_id as site_id,esi.name as site_name " + 
...
/*	String sql = "select teci.name,eet.name,emi.name,ki.name,t.id, t.room_no, t.course_id,t.room_num, t.kaoqu_id, t.shenfen_id, t.edu_type_id, t.major_id, t.grade_id, t.teacher, t.address" +
			" from test_examroom_info t,test_examcourse_info teci,entity_edu_type eet,entity_major_info emi,entity_kaoqu_info ki" + 
			"  where t.batch_id ='" + activeBatchId +"'" + 
...
*/
	...
	public List getStudent(Page page, String kaoqu_id,String site_id, String edutype_id,
		...
		sql = "select distinct a.login_id as regno, b.name as name,b.id as id,b.status as status ,c.name as major_name,"
				...
				+ "f.name as edutype_name,f.id as edutype_id  from test_user_info a, entity_student_info b,entity_kaoqu_info ki,"
				+ "entity_major_info c,entity_site_info d,entity_grade_info e,entity_edu_type f,test_examuser_batch g where "
 ...
 */
	public List getBatches(Page page, List searchproperty, List orderproperty) {
		dbpool db = new dbpool();
		String sql = "select id,name,to_char(s_date,'yyyy-mm-dd') as s_date,to_char(e_date,'yyyy-mm-dd') as e_date,status,to_char(examroom_s_date,'yyyy-mm-dd') as examroom_s_date,to_char(examroom_e_date,'yyyy-mm-dd') as examroom_e_date from test_exambatch_info";
	 ...
	 * @return �õ��������
	 */	
	public List getReBatches(Page page, List searchproperty, List orderproperty) {
		dbpool db = new dbpool();
	...
	public List getExamCourses(String user_id) {
		dbpool db = new dbpool();
		MyResultSet rs = null;
//		String sql = "select b.name as course_name,to_char(b.s_date,'yyyy-mm-dd hh24:Mi:ss') as s_date,to_char(b.e_date,'yyyy-mm-dd hh24:Mi:ss') as e_date from test_examuser_course a,test_examcourse_info b where a.course_id=b.id and a.user_id='"
//				+ user_id + "'";
		String sql = 
			...
			" from test_examuser_course a,test_examcourse_info b,test_examsequence_info c where a.course_id=b.id and c.testbatch_id = b.test_batch_id" + 
	...
	public int addTestCourse(String open_course_id, String es_id,
		...
		ExamSequence es = this.getExamSequence(es_id, batch_id);
	...
	public ExamSequence getExamSequence(String bs_id, String batch_id) {
		dbpool db = new dbpool();
		MyResultSet rs = null;
...
//			}
			int testCourseId = addTestCourse(open_course_id, basicsequence_id,
				...
				i += db.executeUpdate(sql);	//System.out.println("sql5->"+sql);
				UserAddLog.setDebug("OracleExamActivity.importStudent(String open_course_id, String course_id,String batch_id) SQL=" + sql + " DATE=" + new Date());
...
//			}
	...
	//		System.out.println("come finally");
			db.close(rss);
			db = null;
	//		System.out.println("come finally2");
		}
		return i;
	}
...
*/				 sql = "select id from test_examcourse_info tei where tei.open_course_id=" + open_course_id ;
					...
					String course_id1 = "";
						while(rss!=null&&rss.next()){
							 course_id1 = rss.getString("id");
						}
				
				sql1 = "delete from test_examuser_course where course_id=" + course_id1;
				sql = "delete from test_examcourse_info where open_course_id = '"+ open_course_id+"'";
				sql2 =
					"update entity_elective" +
					"   set delay = '0'" + 
					"  where id in (select ee.id as id" + 
					"                from entity_elective      ee," + 
					"                     entity_teach_class   etc," + 
					"                     entity_course_active eca" + 
					"                  where ee.teachclass_id = etc.id" + 
					"                 and etc.open_course_id = eca.id" + 
					"                 and eca.id = '"+open_course_id+"')";

				List sqll = new ArrayList(2);
				sqll.add(sql1);sqll.add(sql);sqll.add(sql2);
...
*/
			 sql = "select id from test_user_info tui where tui.login_id= '" + stu_reg +"'";
			 rss = db.executeQuery(sql);
...
//			String sql1 = "delete from test_examuser_course where user_id='" + user_id+"'";//��ʼ���˰ѣ�ֻ�ǰ����ſε�ɾ��
				String sql1 = 
					"delete from test_examuser_course  where id in (select t.id from test_examuser_course t,test_examcourse_info te" +
					" where t.user_id = '"+user_id+"' and te.open_course_id = '"+open_course_id+"' and t.course_id = te.id)";


				//String sql2 = "delete from test_examuser_batch where user_id='" + user_id+"'";
//				sql = "delete from test_user_info where login_id = '"+ stu_reg+"'";
				String sql3 = "update entity_elective set delay='1' where id in(select ee.id as id from entity_elective ee,entity_teach_class etc,entity_course_active eca , entity_student_info esi "
					+ " where ee.teachclass_id = etc.id and etc.open_course_id = eca.id and eca.id = '"
					+ open_course_id
					+"' and ee.student_id = esi.id and esi.reg_no = '"
					+ stu_reg + "')";
		...
		//		 sqll.add(sql);
				...
				//i += db.executeUpdate(sql);
				 i = db.executeUpdateBatch(sqll);
				UserAddLog.setDebug("OracleExamActivity.delayStudent(String open_course_id, String course_id,String batch_id) SQL=" + sql + " DATE=" + new Date());
		} catch (Exception e) {
		} finally {
			db.close(rss);
			db = null;
	...
	public int setSiteStudent(String site_id, String course_id, String batch_id)
		...
		sql0 = "update test_examuser_course  set room_id='',desk_no='' where course_id='"
				...
				+ batch_id
				+ "' and user_id in (select id from entity_student_info where site_id = '"
				+ site_id + "'))";// �ȰѶ�Ӧ(����ѧ��γ�) ѧ���id���Ҫ��Ĳ��ҿγ�idҲ�Եġ�
		db.executeUpdate(sql0);
		...
		sqlroom = "update test_examroom_info  set room_no='' where course_id='"
				+ course_id + "' and site_id = '" + site_id + "'";//�ѿ���(room)����Ϣ��roomno�Ϳγ�id�Ž�ȥ��
		db.executeUpdate(sqlroom);
		...
		sql1 = "select id,address,room_no,room_num from test_examroom_info where course_id='"
				+ course_id + "' and site_id = '" + site_id + "' order by id";//�ѿ���(room)�е���Ϣ�ҳ�4��
		EntityLog.setDebug(sql1);
		try {
			rs1 = db.executeQuery(sql1);
			if (rs1 != null && rs1.next()) {
				roomId = rs1.getString("id");//�õ�����id���Ϳ����Ŀ������ɵ�����
				capacity = rs1.getInt("room_num");
			}
			sqluser = "select a.id as id from test_examuser_course a,test_examuser_batch b,entity_student_info c where a.user_id=b.user_id and b.batch_id='"
					...
					+ site_id
					+ "') and a.user_id=c.id order by c.grade_id,c.major_id,reg_no";//�������Ա�ҳ�4��
			rsuser = db.executeQuery(sqluser);
			int DESK_NO = 1;
			while (rsuser != null && rsuser.next()) {//������˵Ļ���Ҫ����λ���ˡ�
				String id = rsuser.getString("id");
				String exesql = "";
				if (DESK_NO <= capacity) {
					exesql = "update test_examuser_course set room_id='"
			...
			while (rs3 != null && rs3.next()) {
				...
				String tmpsql = "update test_examroom_info set room_no='" + ROOM_NO + "' where id='" + rs3.getString("id") + "'";//�����˿�����š�
 ...
 */
	...
	//	MyResultSet rss = null;
		...
		for(int i=0;i<ids.length;i++){//�Ѹ��room_Id�еĿ��Ե��������洢��4��
			String num = "";
			String sql = "select room_num as room_num from test_examroom_info t where id='"+ids[i]+"'";
			rs = db.executeQuery(sql);
			try {
				while(rs!=null&&rs.next()){
		...
		for(int i=0;i<ids.length;i++){
			String sql ="select t.desk_no as desk_no,t.id as course_user_id  from test_examuser_course t  where room_id ='"+ids[i]+"' order by t.desk_no";
			String desk_no_list ="";
			...
			try {
				while(rs!=null&&rs.next()){
					...
					if(desk_no_new<=9){
					 ...
					 String sql1 =  "update test_examuser_course set desk_no='"+desk_no_new_str+"' where id ='"+course_user_id+"'";
		...
		String sql="update  test_examroom_info t set t.connect_status='1', t.room_no=(select room_no from test_examroom_info where id='"+ids[0]+"') where t.id in ("+str+")";
...
*/
	 ...
	 * @throws PlatformException
	 */
	public int searchReExamCourseNum(List searchproperty, String batch_id,String basicsequence_id)
	throws PlatformException {
		dbpool db = new dbpool();
		String sql = "select distinct course_id,course_name,credit,course_time,exam_type from (select a.id as course_id,a.name as course_name, a.credit as credit ,a.course_time as course_time,nvl(d.exam_type,'9') as exam_type from (select id ,name,credit, course_time from entity_course_info) a, (select course_id,exam_type,basicsequence_id  from expendtest_examcourse_info where test_batch_id='"
			+ batch_id + "' and basicsequence_id= '" + basicsequence_id  
			+ "') d ,(select id from test_basicsequence_info  ) tbs,(select id,basicsequence_id from expendtest_examsequence_info) tes where   tes.basicsequence_id = tbs.id and tbs.id = d.basicsequence_id and a.id = d.course_id)"
			+ Conditions.convertToCondition(searchproperty, null);
		return db.countselect(sql);
}
	...
	throws PlatformException {
		...
		String sql = "select distinct course_id,opencourse_id,course_name,semester_id,semester_name,credit,course_time,exam_type from (select a.id as course_id,b.id as opencourse_id,a.name as course_name,c.id as semester_id, a.credit as credit ,a.course_time as course_time,c.name as semester_name,nvl(d.exam_type,'9') as exam_type from (select id ,name,credit, course_time from entity_course_info) a,entity_course_active b,entity_semester_info c, (select open_course_id,exam_type,basicsequence_id  from test_examcourse_info where test_batch_id='"
			+ batch_id + "' and basicsequence_id= '" + basicsequence_id  
			+ "') d ,(select id from test_basicsequence_info  ) tbs,(select id,basicsequence_id from test_examsequence_info) tes where a.id=b.course_id and b.semester_id=c.id and b.id=d.open_course_id and tes.basicsequence_id = tbs.id )"//and tbs.id = d.basicsequence_id )"// tes.id = d.basicsequence_id )"
			+ Conditions.convertToCondition(searchproperty, null);
		return db.countselect(sql);
}
...
/*		
		String sql = "select a.id as course_id,d.id as opencourse_id,a.name as course_name, c.id as semester_id,c.name as semester_name,a.credit as credit ,a.course_time as course_time"
			+ " from (select id ,name,credit, course_time from entity_course_info) a,entity_semester_info c,( select eca.id as id,eca.course_id as course_id,eca.semester_id as semester_id from entity_course_active eca where eca.id not in(select open_course_id from test_examcourse_info where test_batch_id ='"+batch_id+"') ) d"
...
*/String sql = "select a.id as course_id,d.id as opencourse_id,a.name as course_name, c.id as semester_id,c.name as semester_name,a.credit as credit ,a.course_time as course_time "
	 ...
	 * @throws PlatformException
	 */
	public int searchCourseNum(List searchproperty, String batch_id,String basicsequence_id)
	throws PlatformException {
		dbpool db = new dbpool();
/*		String sql = "select a.id as course_id,d.id as opencourse_id,a.name as course_name, c.id as semester_id,c.name as semester_name,a.credit as credit ,a.course_time as course_time"
			+ " from (select id ,name,credit, course_time from entity_course_info) a,entity_semester_info c,( select eca.id as id,eca.course_id as course_id,eca.semester_id as semester_id from entity_course_active eca where eca.id not in(select open_course_id from test_examcourse_info ) ) d"
			+ " where a.id = d.course_id and d.semester_id = c.id"
*/		String sql = "select distinct eci.id as course_id,eci.credit as course_credit,eci.course_time as time,eci.name as course_name,eca.id as opencourse_id"
					+" from entity_course_info eci,entity_course_active eca,entity_elective ee,entity_teach_class etc" 
					+" where eci.id = eca.course_id and ee.teachclass_id = etc.id and  etc.open_course_id = eca.id" 
					+ Conditions.convertToCondition(searchproperty, null);
		return db.countselect(sql);
}
...
/*		String sql = "select distinct course_id,examcourse_id,opencourse_id,course_name,semester_id,semester_name,credit,course_time,exam_type from (select d.id as examcourse_id,a.id as course_id,b.id as opencourse_id,a.name as course_name,c.id as semester_id, a.credit as credit ,a.course_time as course_time,c.name as semester_name,nvl(d.exam_type,'9') as exam_type from (select id ,name,credit, course_time from entity_course_info) a,entity_course_active b,entity_semester_info c, (select id,open_course_id,exam_type,test_batch_id,basicsequence_id  from test_examcourse_info where test_batch_id='"
		...
		String sql = "select distinct course_id,examcourse_id,opencourse_id,course_name,semester_id,semester_name,credit,course_time,exam_type from (select d.id as examcourse_id,a.id as course_id,b.id as opencourse_id,a.name as course_name,c.id as semester_id, a.credit as credit ,a.course_time as course_time,c.name as semester_name,nvl(d.exam_type,'9') as exam_type from (select id ,name,credit, course_time from entity_course_info) a,entity_course_active b,entity_semester_info c, (select t.id,t.open_course_id,t.exam_type,t.test_batch_id,t.basicsequence_id  from test_examcourse_info t,test_basicsequence_info tbs where test_batch_id='"
				...
				*/
		String sql = "select distinct course_id,examcourse_id,opencourse_id,course_name,semester_id,semester_name,credit,course_time,exam_type from (select d.id as examcourse_id,a.id as course_id,b.id as opencourse_id,a.name as course_name,c.id as semester_id, a.credit as credit ,a.course_time as course_time,c.name as semester_name,nvl(d.exam_type,'9') as exam_type from (select tt1.id as id, tt1.name as name, tt2.credit as credit,tt2.coursetime as  course_time from entity_course_info tt1,entity_teachplan_course tt2 where  tt1.id = tt2.course_id) a,entity_course_active b,entity_semester_info c, (select t.id,t.open_course_id,t.exam_type,t.test_batch_id,t.basicsequence_id  from test_examcourse_info t,test_basicsequence_info tbs where test_batch_id='"
			+ batch_id + "' and basicsequence_id= '" + basicsequence_id  
			+ "' and t.basicsequence_id = tbs.id) d ,(select t.id,t.basicsequence_id from test_examsequence_info t,test_basicsequence_info tbs where t.basicsequence_id = tbs.id) tes where a.id=b.course_id and b.semester_id=c.id and b.id=d.open_course_id)"// and tes.basicsequence_id = tbs.id and tbs.id = d.basicsequence_id )"//tbs.id = d.basicsequence_id )"
			+ Conditions.convertToCondition(searchproperty, orderproperty);
		MyResultSet rs = null;
...
*/			
	 ...
	 * @throws PlatformException
	 */
	public List searchCourse(Page page, List searchproperty,
			List orderproperty, String batch_id,String basicsequence_id) throws PlatformException {
		dbpool db = null;
/*		String sql = "select a.id as course_id,d.id as opencourse_id,a.name as course_name, c.id as semester_id,c.name as semester_name,a.credit as credit ,a.course_time as course_time "
			+ " from (select id ,name,credit, course_time from entity_course_info) a,entity_semester_info c,( select eca.id as id,eca.course_id as course_id,eca.semester_id as semester_id from entity_course_active eca where eca.id not in(select open_course_id from test_examcourse_info ) ) d"
			+ " where a.id = d.course_id and d.semester_id = c.id"
*/		
		String sql = "select course_id ,credit,course_time,course_name from (select  eci.id as course_id,eci.credit as credit,eci.course_time as course_time,eci.name as course_name,eca.id as opencourse_id"
			+" from entity_course_info eci,entity_course_active eca,entity_elective ee,entity_teach_class etc" 
			+" where eci.id = eca.course_id and ee.teachclass_id = etc.id and  etc.open_course_id = eca.id)" 
			+ Conditions.convertToCondition(searchproperty, orderproperty);
		MyResultSet rs = null;
...
*/
			...
			String sql = 	"select course_user_id,user_id,desk_no,user_name from (select t.id as course_user_id,t.user_id as user_id,t.desk_no as desk_no,u.name as user_name" +
				" from test_examuser_course t ,test_user_info u where room_id='"+testroomId+"' and u.id = t.user_id) order by desk_no";
...
*/				
			String sql = 
				...
				"  from test_examuser_course t ,test_user_info u,test_examroom_info r  where r.room_no='"+testroomno+"' " + 
		...
		public List getExamroomDisplyInfo2
		(String testroomId) {
			dbpool db = new dbpool();
			MyResultSet rs = null;
			List roomList = new ArrayList();
...
/*			String sql = 
				...
				"               u.name         as user_name" + 
				"          from test_examuser_course t," + 
				"               entity_major_info    emi," + 
				"               entity_grade_info    egi," + 
				"               entity_edu_type      eet," + 
				"               test_user_info       u,test_examroom_info r," + 
