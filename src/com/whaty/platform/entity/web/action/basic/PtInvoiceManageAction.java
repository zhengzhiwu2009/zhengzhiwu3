...
public class PtInvoiceManageAction extends MyBaseAction<PeBzzInvoiceInfo> {

	private String id; // 发票id号
	private String invoiceCode;
	private String invoiceFpdm;
	private String invoiceFpjym;
	...
	public void initGrid() {
		...
		this.getGridConfig().setTitle(this.getText("电子发票管理"));
...
//		this.getGridConfig().addColumn(this.getText("邮政编码"), "zipCode", true);
		this.getGridConfig().addColumn(this.getText("购买方手机号码"), "gmfsjhm", true);
		...
		this.getGridConfig().addColumn(this.getText("合并订单号"), "peBzzOrderInfo.mergeSeq", false);
		this.getGridConfig().addColumn(this.getText("发票代码"), "fpdmNum", false);
		this.getGridConfig().addColumn(this.getText("发票号码"), "num", true);
		this.getGridConfig().addColumn(this.getText("发票校验码"), "fpjymNum", false);
...
//
			this.getGridConfig().addMenuScript(
					this.getText("Excel批量导入发票号、发票代码、发票校验码"),
...
//					.getGridConfig()
//					.addMenuScript(
//							this.getText("写入发票号码、发票代码、发票校验码"),
//							"{alert('写入发票号码、发票代码、发票校验码分别用“、”隔开'); "
//									+
//
...
//									+ "        url:'/entity/basic/ptInvoiceManage_writeCode.action',"
//									+ "        waitMsg:'处理中，请稍候...',"
...
//									+ "   });" + "   addModelWin.show();" + "}");
		...
		.addMenuScript(
				...
				"{var m = grid.getSelections();  "
						...
						+ "	document.getElementById('user-defined-content').innerHTML=\"<form target='' action='/entity/basic/ptInvoiceManage_toWriteCode.action' method='post' name='formx1' style='display:none'><input type=hidden name=ids value='\"+jsonData+\"' ></form>\";"
...
//									+ "	}                        "
//									+ "	document.getElementById('user-defined-content').style.display='none'; "
//									+ "	document.getElementById('user-defined-content').innerHTML=\"<form target='_blank' action='/entity/basic/invoiceManage_printInvoice.action' method='post' name='formx1' style='display:none'><input type=hidden name=ids value='\"+jsonData+\"' ></form>\";"
//									+ "	document.formx1.submit();"
//									+ "	document.getElementById('user-defined-content').innerHTML=\"\";"
//									+ "} else {                    "
...
//						"id");
	...
	public void setServletPath() {
		// TODO Auto-generated method stub
		this.servletPath = "/entity/basic/ptInvoiceManage";
	}
	...
	public String writeCode() {
		...
		String fpjym = this.invoiceFpjym;
		try {
			DetachedCriteria dc = DetachedCriteria
					.forClass(PeBzzInvoiceInfo.class);
			dc.createCriteria("peBzzOrderInfo", "peBzzOrderInfo");
			...
			/*map.clear();
			map.put("success", "false");
			map.put("info", "故障，写入失败");
			this.setJsonString(JsonUtil.toJSONString(map));
			return json();*/
			this.setMsg("发票号码、发票代码、发票校验码写入失败");
			this.setTogo(this.getServletPath() + ".action");
			return "msg";
		}

		this.setMsg("发票号码、发票代码、发票校验码写入成功");
		this.setTogo(this.getServletPath() + ".action");
		return "msg";
	}
	...
	public DetachedCriteria initDetachedCriteria() {
		...
		dc.add(Restrictions.eq("peBzzOrderInfo.enumConstByFlagPaymentState",
				enumConstByFlagPaymentState));
		dc.add(Restrictions.eq("enumConstByInvoiceType", enumConstByInvoiceType));
		try {
			// 处理时间的查询参数
	...
	public String getInvoiceFpjym() {
		return invoiceFpjym;
	}

	public void setInvoiceFpjym(String invoiceFpjym) {
		this.invoiceFpjym = invoiceFpjym;
	}
}
