...
public class LMSCMIServletJS extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		/*Map<String,Object> paras = req.getParameterMap();
		for(String s:paras.keySet()){
			Object obj = paras.get(s);
			System.out.print("key:"+s+",,,,,,,,value:");
			if(obj instanceof String){
				System.out.println(obj);
			}else{
				System.out.println(Arrays.asList(((String[])obj)));
			}
		}*/
		...
		if("getdata".equalsIgnoreCase(command)){
			...
			String suspendData = scoData.getSuspendData().getSuspendData().getValue();
			if(suspendData == null){
				suspendData = "";
			}
			...
			sb.append(dataModel[16]+"|");sb.append(suspendData+";");
			
			//System.out.println("*****getdata in LMSCMIServletJS*******="+sb.toString());
			...
			for(int i =0 ;i <dmeKey.length;i++){
				if(dataModel[0].equals(dmeKey[i])){
					...
					//处理掉undefined值
					if(dmeValue[i].indexOf("undefined") > 0){
						String[] tempArray = dmeValue[i].split(",");
						StringBuffer sb = new StringBuffer();
						for(int j=0;j<tempArray.length;j++){
							if(!tempArray[j].equals("undefined")){
			...
			String messages = "" + (new Date().getTime());
			out.write(messages);
