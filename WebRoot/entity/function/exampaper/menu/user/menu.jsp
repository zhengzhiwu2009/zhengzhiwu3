...
<%
	try {
		...
		HashMap Score = new HashMap();//是从这里把分数存进去的。
		...
		for(int i=0; i<questionList.size(); i++) {
			...
			String pq_type = pq.getType(); //String scoretest = pq.getReferenceScore(); 
//			String scoretestbefore = pq.getScore(); 
			if(!pq_type.equalsIgnoreCase(TestQuestionType.YUEDU))
			{
				Score.put(pq.getId(),pq.getScore());//这个是以前的，存进去为100分的。
				//System.out.println(pq.getScore() + "---------------");
//			    Score.put(pq.getId(),pq.getReferenceScore());
			...
			if(pq_type.equalsIgnoreCase(TestQuestionType.YUEDU))
			{
				...
				scoreList.add(pq.getScore()); //这个是以前的存进去怎么就成了100分的。
//				scoreList.add(pq.getReferenceScore()); 
...
<%
	...
	<%
		...
		{
	%>
	parent.parent.submain.location= "../../question_info.jsp";
	<%
		}
