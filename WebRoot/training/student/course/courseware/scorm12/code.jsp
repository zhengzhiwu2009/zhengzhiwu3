...
{  
	...
	%>var MTMenuText = "<%=course_title%>";
	  ...
	  menu.MTMAddItem(new MTMenuItem("<%=title_vector.elementAt(i)%>", "javascript:launchItem('<%=id_vector.elementAt(i).toString()%>')", "code"));
	  <% 
	  ...
	  // if nesting level of current item is same as that of previous item
	        ...
	        { %>
	            ...
	            <%=parent_vector.elementAt(parent_index).toString()%>.MTMAddItem(new MTMenuItem("<%=title_vector.elementAt(i)%>", "<%=action%>", "code"));
	        ...
	        {  
	           ...
	           item_number_vector.addElement("0");
	           current_index++;%>                 
	           <%=menu_name%>.MTMAddItem(new MTMenuItem("<%=title_vector.elementAt(i)%>", "<%=action%>", "code")); 
	           <%
	           previous_level = level_vector.elementAt(i++).toString();
	        } //end else if
	        else
...
<body onload="MTMStartMenu()" bgcolor="#E2EEF3" text="#black"
