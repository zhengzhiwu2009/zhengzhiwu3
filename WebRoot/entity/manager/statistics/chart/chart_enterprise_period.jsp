...
<%
		...
		String stime ="";
		String etime ="";
		String temp = fixnull(request.getParameter("d"));
		StringTokenizer tokenizer = new StringTokenizer(temp,"|");
		String [] teng = new String [tokenizer.countTokens()];
		int k =0;
		while(tokenizer.hasMoreTokens()){
			teng[k] = tokenizer.nextToken();
			k++;
		}
		batch_id = teng[0];
		enterprise_id = teng[1];
		stime = teng[2];
		etime = teng[3];
	   ...
	   StringBuffer sqltemp=new StringBuffer();
		String sql ="  select st.start_date as sdate , st.end_date as edate "
			...
			+   "  where ps.fk_sso_user_id = st.fk_ssouser_id and ps.fk_enterprise_id ='"+enterprise_id
			+   "'  and ps.fk_batch_id ='"+batch_id+"' and ps.fk_sso_user_id=u.id and u.flag_isvalid='2' and ps.flag_rank_state='402880f827f5b99b0127f5bdadc70006' and (st.start_date between to_date('"+stime+"','yyyy-MM-dd hh24:mi:ss') and to_date('"+etime+"','yyyy-MM-dd hh24:mi:ss')" 
			+   "  or st.end_date between to_date('"+stime+"','yyyy-MM-dd hh24:mi:ss') and to_date('"+etime+"','yyyy-MM-dd hh24:mi:ss'))";
       ...
       String str =" <chart  xAxisName='Ê±¼ä£¨Ê±£©' yAxisName='Number'   showValues='0' alternateHGridColor='FCB541' alternateHGridAlpha='20' divLineColor='FCB541' divLineAlpha='50' canvasBorderColor='666666' baseFontColor='666666' lineColor='FCB541'>"+
