...
public class TeacherGradeAction extends MyBaseAction<TeacherInfo>  {
	private String year;
	...
	public void setServletPath() {
		this.servletPath = "/entity/basic/teacherGrade";
	}
	...
	public Page list() { 
		...
		try {
			sb.append("SELECT * FROM (SELECT STG.TEACHER_ID AS ID, STG.TEACHER_NAME, ");
			sb.append("       NVL(STG.SKSP_GRADE, 0) FIR, NVL(STG.XXXG_GRADE, 0) SIX, ");
			sb.append("       NVL(STG.BMRC_GRADE, 0) REG_NUM_GRA, NVL(STG.SUM_GRADE, 0) SUM_GRA ");
			sb.append("  FROM STATISTIC_TEACHER_GRADE STG ");
			sb.append(" WHERE STG.VOTE_DATE = '" + year + "') A WHERE 1=1 ");
			if(teacherName != null && !"".equals(teacherName)) {
				sb.append(" AND A.TEACHER_NAME LIKE '%" + URLDecoder.decode(teacherName, "UTF-8") + "%'");
			}
			/* 拼接查询条件 */
	...
	public String toSearchTeacherGrade() {
		return "to_search_teacher_grade";
	}
	
	public String searchTeacherGrade() {
