...
<%
	...
	for (int item = 0; item < peBzzCertificates.size(); item++) 
	{
		...
		//取得学期id号
		//String batch_id = fixnull(((PeBzzCertificate)peBzzCertificates.get(item)).getPeBzzStudent().getPeBzzBatch().getId());
		//String batch_start_date = fixnull(new java.text.SimpleDateFormat("yyyy年MM月").format(((PeBzzCertificate)peBzzCertificates.get(item)).getPeBzzStudent().getPeBzzBatch().getStartDate()));
		//String batch_end_date = fixnull(new java.text.SimpleDateFormat("yyyy年MM月").format(((PeBzzCertificate)peBzzCertificates.get(item)).getPeBzzStudent().getPeBzzBatch().getEndDate()));
		String batch_id = "";
		String batch_start_date = "";
		String batch_end_date = "";
		String sql_batch = "select pbb.id as batch_id,pbb.start_time as batch_start_date,pbb.end_time as batch_end_date \n"+
							" from pe_bzz_batch pbb,pe_bzz_student pbs \n"+
							" where pbb.id=pbs.fk_batch_id and pbs.id='"+stu_id+"'";
		rs = db.executeQuery(sql_batch);
		while(rs!=null&&rs.next()){
			batch_id = fixnull(rs.getString("batch_id"));
			batch_start_date = new java.text.SimpleDateFormat("yyyy年MM月").format(rs.getDate("batch_start_date"));
			batch_end_date = new java.text.SimpleDateFormat("yyyy年MM月").format(rs.getDate("batch_end_date"));
		}
	 	db.close(rs);
