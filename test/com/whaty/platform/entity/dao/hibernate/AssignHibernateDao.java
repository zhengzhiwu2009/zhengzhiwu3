...
public class AssignHibernateDao<T extends AbstractBean> extends HibernateDaoSupport implements AssignDao<T> {

	private Class entityClass;

	private static final Log log = LogFactory.getLog(AssignHibernateDao.class);
	...
	public Page getByPageSQL(String sql, int pageSize, int startIndex) {
		...
		String mySql = "select * from ( " + "select  a.*, rownum rownum_ from ( " + sql + ") a where rownum <= " + (startIndex + pageSize)
	...
	public void saveTrainingHis(String userId, Object courseId, Object opencourseId, Date startTime, Date endTime) throws Exception {
		...
		try {
			String fkOrderIdSql = "SELECT FK_ORDER_ID FROM PR_BZZ_TCH_STU_ELECTIVE WHERE FK_STU_ID = (SELECT ID FROM PE_BZZ_STUDENT WHERE FK_SSO_USER_ID = '"
					+ userId + "') AND FK_TCH_OPENCOURSE_ID = '" + opencourseId + "'";
			...
			sBuffer
					.append(" SELECT ID FROM TRAINING_COURSE_STUDENT_HIS WHERE FK_STU_ID = (SELECT ID FROM PE_BZZ_STUDENT WHERE FK_SSO_USER_ID = '"
							+ userId + "') ");
			sBuffer.append(" AND COURSE_ID = '" + opencourseId + "' ");
			if (!"".equals(fkOrderId)) {
				sBuffer
						.append(" AND ROUND(TO_NUMBER(TO_DATE('"
								+ st
								+ "','yyyy-MM-dd hh24:mi:ss') - (SELECT MAX(START_TIME) FROM TRAINING_COURSE_STUDENT_HIS WHERE FK_STU_ID = (SELECT ID FROM PE_BZZ_STUDENT WHERE FK_SSO_USER_ID = '"
								+ userId + "') AND COURSE_ID = '" + opencourseId + "' AND FK_ORDER_ID = '" + fkOrderId
								+ "')) * 24 * 60 * 60) < 30 ");
				...
				sBuffer
						.append(" AND ROUND(TO_NUMBER(TO_DATE('"
								+ st
								+ "','yyyy-MM-dd hh24:mi:ss') - (SELECT MAX(START_TIME) FROM TRAINING_COURSE_STUDENT_HIS WHERE FK_STU_ID = (SELECT ID FROM PE_BZZ_STUDENT WHERE FK_SSO_USER_ID = '"
								+ userId + "') AND COURSE_ID = '" + opencourseId + "' AND FK_ORDER_ID IS NULL)) * 24 * 60 * 60) < 30 ");
			...
			if (null != searchID && !"null".equalsIgnoreCase(searchID) && !"".equals(searchID)) {
				updateOrInsertSql = "UPDATE TRAINING_COURSE_STUDENT_HIS SET START_TIME = TO_DATE('" + st
						+ "','yyyy-MM-dd hh24:mi:ss'),END_TIME = TO_DATE('" + et + "','yyyy-MM-dd hh24:mi:ss'),TOTAL_TIME = '" + totalTime
						+ "' WHERE ID = '" + searchID + "'";
			} else {// 不存在插入数据
				updateOrInsertSql = "INSERT INTO TRAINING_COURSE_STUDENT_HIS (ID,FK_STU_ID,COURSE_ID,START_TIME,END_TIME,TOTAL_TIME,FK_ORDER_ID) SELECT SEQ_TCS_HIS.NEXTVAL, ID,'"
						+ opencourseId
						+ "',TO_DATE('"
						+ st
						...
						+ totalTime + "','" + fkOrderId + "' FROM PE_BZZ_STUDENT WHERE FK_SSO_USER_ID = '" + userId + "'";
			}
