...
import com.whaty.platform.database.oracle.standard.test.OracleReTestUser;
...
import com.whaty.platform.test.reexam.ReExamList;
...
public class OracleReExamList implements ReExamList {
	private String SQLBAT = "select id,name,to_char(s_date,'yyyy-mm-dd') as s_date,to_char(e_date,'yyyy-mm-dd') as e_date,status,to_char(examroom_s_date,'yyyy-mm-dd') as examroom_s_date,to_char(examroom_e_date,'yyyy-mm-dd') as examroom_e_date from expendtest_exambatch_info";

	private String SQLBATACTIVE = "select id,name,to_char(s_date,'yyyy-mm-dd') as s_date,to_char(e_date,'yyyy-mm-dd') as e_date,status,to_char(examroom_s_date,'yyyy-mm-dd') as examroom_s_date,to_char(examroom_e_date,'yyyy-mm-dd') as examroom_e_date from expendtest_exambatch_info where status='1'";
	
	private String SQLCOURSE = "select id,name,to_char(s_date,'yyyy-mm-dd hh24:Mi:ss') as s_date,to_char(e_date,'yyyy-mm-dd hh24:Mi:ss') as e_date,test_batch_id,course_id,EXAMSEQUENCE_ID from expendtest_examcourse_info";
	...
	private String SQLROOM = "select id,name,address,room_no,course_id  from expendtest_examroom_info";

	private String SQLEXAMUSER = "select id,batch_id,user_id,examcode,note,status from expendtest_examuser_batch where id <>'0'";
	...
	public int getBatchNum(List searchproperty) {
		...
		String sql = "";
		sql = SQLBAT + Conditions.convertToCondition(searchproperty, null);
	...
	public int getBatchNumStatus(List searchproperty) {
		dbpool db = new dbpool();
		String sql = "";
		sql = "select count(*) as num from expendtest_exambatch_info t where t.status='1'" + Conditions.convertToCondition(searchproperty, null);
		//int i = db.countselect(sql);
		MyResultSet rs = null;
...
*/
			String sql = 
				"select kaoqu_id, grade_id,  semester_id, edutype_id,  shenfen_id, major_id, opencourse_id, course_name, room_no, room_num,address, teacher " +
				"  from (select t.kaoqu_id        as kaoqu_id, " + 
				"               t.grade_id        as grade_id, " + 
				"               ti.id            as semester_id," + 
				"               t.edu_type_id     as edutype_id, " + 
				"               t.shenfen_id      as shenfen_id, " + 
				"               t.major_id        as major_id," + 
				"               ti.course_id as opencourse_id," + 
				"               ti.name           as course_name," + 
				"               t.room_no         as room_no," + 
				"               t.room_num        as room_num," + 
				"               t.address         as address," + 
				"               t.teacher         as teacher" + 
				"          from expendtest_examroom_info   t," + 
				"               expendtest_examcourse_info ti " + 
				"         where t.course_id = ti.id" + 
...
//kaoqu_id,grade_id,,edutype_id,shenfen_id,major_id,opencourse_id,course_name,semester_id,room_no,room_num,address,teacher 
				     ...
				     studentMsg.add(rs.getString("opencourse_id"));//5��Ȼ���ֽ���opencourse_id,ʵ�ʴ洢����course_id
...
//				System.out.println(e.toString());
	...
	public List getCourses(Page page, List searchproperty, List orderproperty) {
		...
		try {
			...
			while (rs != null && rs.next()) {
				...
				oracleExamCourse.setExamBatch(new OracleReExamBatch(rs
	...
	public List getSiteCourses(String site_id, Page page, List searchproperty,
		...
		String sql = "select id,name,s_date,e_date,test_batch_id,course_id,examsequence_id,examsequence_title from (select a.id,a.name,to_char(a.s_date,'yyyy-mm-dd hh24:Mi:ss') as s_date,"
				...
				+ "from expendtest_examcourse_info a,expendtest_examsequence_info b where a.examsequence_id=b.id and a.id in (select course_id from expendtest_examuser_course "
		...
		try {
			...
			while (rs != null && rs.next()) {
				...
				oracleExamCourse.setExamBatch(new OracleReExamBatch(rs
						.getString("test_batch_id")));
				oracleExamCourse.setCourse_id(rs.getString("course_id"));
				OracleReExamSequence examSequence = new OracleReExamSequence();
	...
	public List getRooms(Page page, List searchproperty, List orderproperty) {
		...
		try {
			...
			while (rs != null && rs.next()) {
				...
				oracleExamRoom.setExamCourse(new OracleReExamCourse(rs
						.getString("course_id")));
	...
	public List getExamUsers(Page page, List searchproperty, List orderproperty) {
		...
		try {
			...
			while (rs != null && rs.next()) {
				...
				oracleExamUser.setExamBatch(new OracleReExamBatch(rs
						.getString("batch_id")));
	...
	public List getExamSequences(Page page, List searchproperty,
		...
		String sql = "select id,title,s_date,e_date,note, testbatch_id,batchname,basicsequence_id from "
				...
				+ "b.name as batchname,a.basicsequence_id from expendtest_examsequence_info a, expendtest_exambatch_info b where a.testbatch_id=b.id and b.status='1')";
		sql = sql
				+ Conditions.convertToCondition(searchproperty, orderproperty);
	...
	*/
			String sql = 
				...
				"            from expendtest_examcourse_info teci, expendtest_examuser_course teuc," + 
				"          expendtest_examuser_batch teub,expendtest_exambatch_info tebi,expendtest_user_info tui," + 
...
//				}
				...
				//			.getString("batch_id")));
					oracleExamUser.setTestUser(new OracleReTestUser(rs
							.getString("user_id")));
	...
	public int getExamSequencesNum(List searchproperty) {
		dbpool db = new dbpool();
		String sql = "select id,title,s_date,e_date,note, testbatch_id from (select id,title,to_char(startdate,'yyyy-mm-dd') as s_date,to_char(enddate,'yyyy-mm-dd') as e_date,note,testbatch_id from expendtest_examsequence_info where testbatch_id in(select id from expendtest_exambatch_info where status='1'))";
	...
	public List getActiveExamSequences(Page page, List searchproperty,
			List orderproperty) {
		dbpool db = new dbpool();
		String sql = "select id,title,startdate,enddate,note,testbatch_id from (select a.id,title,to_char(a.startdate,'yyyy-mm-dd') as startdate,to_char(a.enddate,'yyyy-mm-dd') as enddate,note,testbatch_id from expendtest_examsequence_info a,expendtest_exambatch_info b where a.testbatch_id=b.id and b.status='1')";
		sql = sql
				+ Conditions.convertToCondition(searchproperty, orderproperty);
		EntityLog.setDebug(sql);
		MyResultSet rs = null;
 ...
 */
	public List getExamCourse(Page page, List searchproperty, List orderproperty) {
		...
		String sql = "select id ,course_name,course_credit,course_time,s_date,e_date,test_batch_id,course_id,examsequence_id,title,batchname,course_type from "
				...
				+ "a.examsequence_id as examsequence_id,c.title as title,d.name as batchname,a.course_type from expendtest_examcourse_info a,"
	...
	public List getSiteExamCourse(String site_id, Page page,
		...
		String sql = "select id ,course_name,s_date,e_date,test_batch_id,course_id,examsequence_id,"
				...
				+ "examsequence_id,c.title as title,d.name as batchname from expendtest_examcourse_info a,"
	...
	public int getExamCourseNum(List searchproperty) {
		...
		String sql = "select id ,course_name,s_date,e_date,test_batch_id,course_id,examsequence_id,"
				...
				+ "examsequence_id,c.title as title,d.name as batchname from expendtest_examcourse_info a,"
				+ "entity_course_active b,expendtest_examsequence_info c,expendtest_exambatch_info d where a.course_id=b.course_id and "
				+ "a.examsequence_id=c.id and a.test_batch_id=d.id)";
		sql = sql + Conditions.convertToCondition(searchproperty, null);
		EntityLog.setDebug(sql);
		int i = db.countselect(sql);
	...
	public List getExamStat(String batch_id, String site_id)
		...
		if (site_id == null || site_id.equals(""))
			sql = "select site_name,grade_name,major_name,edutype_name,course_name,examsequence_name,"
					...
					+ "c.site_id,c.grade_id,c.major_id,c.edutype_id,b.room_id,count(b.id) as num from expendtest_examcourse_info a,expendtest_examuser_course b,"
	...
	public List getExamStat(String batch_id, String site_id, String order)
		...
		if (site_id == null || site_id.equals(""))
			sql = "select site_name,grade_name,major_name,edutype_name,course_name,examsequence_name,"
					...
					+ "c.site_id,c.grade_id,c.major_id,c.edutype_id,count(b.id) as num from expendtest_examcourse_info a,expendtest_examuser_course b,"
					+ "entity_student_info c where a.test_batch_id='"
					+ batch_id
					+ "' and a.id=b.course_id and b.user_id=c.id group by a.name,a.id,c.site_id,"
		...
		try {
			...
			while (rs != null && rs.next()) {
				...
				String addSql = "select address,room_num from (select distinct address as address,room_num from expendtest_examroom_info a,expendtest_examcourse_info b,"
	...
	public List getBasicSequencesNoTime(Page page, List searchproperty,
		...
		" select basicsequence_id from expendtest_examsequence_info )"
				+ Conditions.convertToCondition(searchproperty, orderproperty);
		MyResultSet rs = null;
		List list = null;
		try {
	...
	public List getBasicSequencesNoTime(Page page, List searchproperty,
		...
		dbpool db = new dbpool();
		String sql = "select id,title,note from test_basicsequence_info where id in(select id from test_basicsequence_info minus " +
		" select t.basicsequence_id from expendtest_examsequence_info t where t.testbatch_id='"+batchId+"')"
				+ Conditions.convertToCondition(searchproperty, orderproperty);
		MyResultSet rs = null;
		List list = null;
		try {
	...
	public List getCourses1(String activeBatchId,Page page, List searchproperty, List orderproperty) {
		dbpool db = new dbpool();
		String sql = "select id,name,to_char(s_date,'yyyy-mm-dd hh24:Mi:ss') as s_date,to_char(e_date,'yyyy-mm-dd hh24:Mi:ss') as e_date,test_batch_id,course_id,basicSEQUENCE_ID from expendtest_examcourse_info where test_batch_id='"+ activeBatchId +"'"
				+ Conditions.convertToCondition(searchproperty, orderproperty);
		MyResultSet rs = null;
		ArrayList oracleExamCourses = null;
		try {
