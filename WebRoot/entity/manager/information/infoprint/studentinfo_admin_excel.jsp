...
<%
	...
	if (!batch_id.equals("")) {
		...
		if(db.countselect(tmp_sql)>0 && erji_id.equals(""))   //表示所选企业为一级企业
		{
		 esql ="select pe.id as id, pe.name as name, pe.code as code, pe.industry as industry, pe.address as address, pe.zipcode as zipcode,\n"
			...
			+ "                 p.lxr_name, p.lxr_xb, p.lxr_position, p.lxr_phone, p.lxr_mobile, p.lxr_email, p.lxr_address from pe_bzz_student s, pe_enterprise p,sso_user su \n"
			+ "                 where p.fk_parent_id is null and s.fk_enterprise_id = p.id and s.fk_sso_user_id=su.id and su.flag_isvalid='2' and s.flag_rank_state='402880f827f5b99b0127f5bdadc70006' "+sql_s+"  union select s.id as sid, p.id as pid,p.name, p.code, p.industry,\n"
			+ "                 p.address,p.zipcode, p.fax, p.fzr_name, p.fzr_xb, p.fzr_position, p.fzr_phone, p.fzr_mobile, p.fzr_email, p.fzr_address,\n"
			+ "                 p.lxr_name, p.lxr_xb, p.lxr_position, p.lxr_phone, p.lxr_mobile, p.lxr_email, p.lxr_address from pe_bzz_student s, pe_enterprise p, pe_enterprise p1,sso_user su \n"
			+ "                 where p.fk_parent_id is null and p1.fk_parent_id = p.id and p1.id = s.fk_enterprise_id and s.fk_sso_user_id=su.id and su.flag_isvalid='2' and s.flag_rank_state='402880f827f5b99b0127f5bdadc70006' "+sql_s+" ) pp right outer join pe_enterprise pe on pe.id = pp.pid\n"
		...
		else  //二级企业
		{
			...
			+ "                 p.address, p.zipcode, p.fax, p.fzr_name, p.fzr_xb, p.fzr_position, p.fzr_phone, p.fzr_mobile, p.fzr_email,p.fzr_address,\n"
			+ "                 p.lxr_name, p.lxr_xb, p.lxr_position, p.lxr_phone, p.lxr_mobile, p.lxr_email, p.lxr_address from pe_bzz_student s, pe_enterprise p,sso_user su \n"
			+ "                 where s.fk_enterprise_id = p.id and s.fk_sso_user_id=su.id and su.flag_isvalid='2' and s.flag_rank_state='402880f827f5b99b0127f5bdadc70006' "+sql_s+" ) pp right outer join pe_enterprise pe on pe.id = pp.pid\n"
	...
	String sql_t = "select rownum , ps.true_name as name  , ps.reg_no as regno ,  ss.password as passwords ,\n"
			...
			+ " ps.email as email,ps.job_number as job_number  from pe_bzz_student ps inner join sso_user ss on ps.fk_sso_user_id = ss.id and ss.flag_isvalid='2' and ps.flag_rank_state='402880f827f5b99b0127f5bdadc70006'\n"
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1183 style='border-collapse:
 ...
 <tr height=30 style='height:22.5pt'>
  <td colspan=14 height=30 class=xl37 width=1283 style='height:22.5pt;
  width:888pt'>学员信息登记表</td>
 </tr>
 
 				<%
 					int n = db.countselect(sql_t);
 ...
 <tr height=26 style='mso-height-source:userset;height:19.5pt'>
  ...
  height:19.5pt'>学员总人数：<span style='mso-spacerun:yes'>&nbsp; </span><%=n%><span
 ...
 <tr height=32 style='mso-height-source:userset;height:24.0pt'>
  ...
  <td rowspan=2 class=xl24 width=91 style='border-top:none;width:68pt'>工号</td>
 </tr>
 <tr height=19 style='height:14.25pt'>
 ...
 <%=pename%>
  ...
  <td class=xl31 width=238 style='border-top:none;border-left:none;width:179pt'><%=fixnull(rs.getString("job_number"))%></td>
 </tr>
 <%
 }
 	db.close(rs);
