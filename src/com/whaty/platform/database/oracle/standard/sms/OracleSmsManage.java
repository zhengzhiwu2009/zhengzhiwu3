...
public class OracleSmsManage extends SmsManage {
	...
	public String sendMessage(SmsMessage message) throws PlatformException {
		if (smsManagerPriv.sendSms == 1) {
			...
			try {
				...
				.setInfo(
						"<whaty>USERID$|$"
								...
								+ "</whaty><whaty>BEHAVIOR$|$sendMessage</whaty><whaty>STATUS$|$"
								+ value
			...
			throw new PlatformException("no right to sendSMS!");
		}
	}
	...
	//记录发送的系统短信 ，czc 2009-1-21
	public int addSystemSmsMessage(String targets, String content,Date time) throws PlatformException {
		...
		smsMessage.setTargets(targets);
		smsMessage.setContent(content);
		smsMessage.setSender("System");
//		smsMessage.setScope(scope);
		smsMessage.setType("2");
		smsMessage.setSiteId("0");
		smsMessage.setSetTime("");
		smsMessage.setTime(sf.format(time));
		smsMessage.setStatus("1");
	...
	public List getSmsMessageNumBySite(Page pageover, String site_id,
	...
	public String getMobiles(String fileName) throws PlatformException {
		if (smsManagerPriv.batchImportMobiles == 1) {
			String mobiles = "";
			try {
			...
			throw new PlatformException("你没有批量导入移动号码的权限!");
		}
	}
	...
	public int updateSmsMessageStatus(String id,String sendStatus) throws PlatformException {
		OracleSmsMessage smsMessage = new OracleSmsMessage();
		smsMessage.setMsgId(id);
		smsMessage.setSendStatus(sendStatus);
		return smsMessage.updateMsgStatus();
	}
