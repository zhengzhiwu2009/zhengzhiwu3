...
/**
 * 公共课程发布Action 2014年6月24日
 * 
 * @author Lee
 * 
 */
public class PeBzzCourseZixuanManagerAction extends MyBaseAction<PeBzzTchCourse> {
	...
	public void initGrid() {
		...
		ColumnConfig columnConfigRecommend = new ColumnConfig(this.getText("推荐状态"), "enumConstByFlagIsRecommend.name", false, true, false, "TextField", false, 100, "");
		...
		this.getGridConfig().addColumn(this.getText("通过规则描述"), "passRoleNote", false, true, false, "");
		...
		if (us.getUserLoginType().equals("3")) {
			...
			if (capabilitySet.contains(this.servletPath + "_setTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("设置讲师图片"), "/entity/teaching/peBzzCourseZixuanManager_setTeacherImg.action", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_removeTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("删除讲师图片"), "/entity/teaching/peBzzCourseZixuanManager_removeImg.action?param=t", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_setSuggest.action")) {
				this.getGridConfig().addMenuFunction(this.getText("设置建议学习人群"), "/entity/teaching/peBzzCourseZixuanManager_setSuggest.action", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_removeSuggest.action")) {
				this.getGridConfig().addMenuFunction(this.getText("删除建议学习人群"), "/entity/teaching/peBzzCourseZixuanManager_removeSuggest.action", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_manageSource.action")) {
				this.getGridConfig().addRenderFunction(this.getText("学习资源"), "<a href=\"/sso/bzzinteraction_InteractioManage.action?course_id=${value}&teacher_id=teacher1\" target='_blank'>学习资源</a>",
						"id");
			}
			if (capabilitySet.contains(this.servletPath + "_impCourse.action") && capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this
						...
						.addRenderScript(
								this.getText("课件文件管理"),
								"{if(record.data['coursewareId'] == ''){return '&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/entity/teaching/peBzzCourseZixuanManager_toUploadCourseware.action?bean.id='+record.data['id']+'\" target=\"_blank\">导入</a>';} else return '<a href=\"#\" onclick=if(true){window.open(\"/entity/manager/course/courseware/filemanager/filemanager_main02.jsp?courseware_id='+record.data['code']+'\")} >管理</a>&nbsp;&nbsp;<a href=\"#\" onclick=if(confirm(\"确定要删除吗\")){window.open(\"/entity/manager/course/courseware/scorm12/delete_scorm_info.jsp?courseware_id='+record.data['code']+'\")} >删除</a>'}",
								"id");
			}
			if (capabilitySet.contains(this.servletPath + "_impCourse.action") && !capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this
						...
						.addRenderScript(
								this.getText("课件文件管理"),
								"{if(record.data['coursewareId'] == ''){return '&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/entity/teaching/peBzzCourseZixuanManager_toUploadCourseware.action?bean.id='+record.data['id']+'\" target=\"_blank\">导入</a>';} else return '<a href=\"#\" onclick=if(confirm(\"确定要删除吗\")){window.open(\"/entity/manager/course/courseware/scorm12/delete_scorm_info.jsp?courseware_id='+record.data['code']+'\")} >删除</a>'}",
								"id");
			}
			if (!capabilitySet.contains(this.servletPath + "_impCourse.action") && capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this
			...
			if (capabilitySet.contains(this.servletPath + "_satified.action")) {
				this.getGridConfig().addRenderFunction(this.getText("满意度调查"),
						"<center><a href=\"/entity/teaching/addSatisfactionPaper.action?courseId=${value}&flag=addPaper&area=peBzzCourseZixuanManager\" >添加</a></center>", "id");
			}
	...
	public void setServletPath() {
		// TODO Auto-generated method stub
		this.servletPath = "/entity/teaching/peBzzCourseZixuanManager";
	}
		...
		if (this.getIds() != null && this.getIds().length() > 0) {
			...
			if (str != null && str.length() > 0) {
				...
				String sql = "SELECT COUNT(ID)  FROM PE_BZZ_BATCH WHERE ID IN (SELECT FK_BATCH_ID FROM PR_BZZ_TCH_OPENCOURSE " + "WHERE FK_COURSE_ID IN (" + idss + ")) "
						+ "and FLAG_BATCH_TYPE IS NOT NULL AND FLAG_BATCH_TYPE != '40288a7b394d676d01394db3990d003e'";

				try {
					System.out.print(this.getGeneralService().getBySQL(sql).get(0));
...
//				System.out.println(count);
	...
	public Page list() {
		...
		try {
			...
			sqlBuffer.append(" EC4.NAME FLAGCONTENTPROPERTY, EC8.NAME FLAGISVALID, EC9.NAME FLAGOFFLINE, EC5.NAME FLAGISEXAM, EC.NAME FLAGISRECOMMEND, ");
			sqlBuffer.append(" EC6.NAME FLAGISFREE, A.PRICE, A.TIME, A.TEACHER, ");
			...
			sqlBuffer.append(" A.PRICE,A.TIME,A.TEACHER,A.STUDYDATES,A.END_DATE,A.ANNOUNCE_DATE, ");
	...
	private StringBuffer getScript() {
		...
		script.append("	                 url:'/entity/teaching/peBzzCourseZixuanManager_updateIndex.action',   ");
	...
	public String courseInfo() {
		...
		try {
			...
			sb.append(" EC8.NAME FLAGISFREE,A.PHOTO_LINK,A.TEA_IMG,A.PASSROLE_NOTE,A.SUGGESTION,CC.INDEXFILE ");
			...
			sb.append(" LEFT JOIN SCORM_SCO_LAUNCH CC ON A.CODE = CC.COURSE_ID ");
			...
			sb.append(" EC7.NAME,EC8.NAME,A.PHOTO_LINK,A.TEA_IMG,A.PASSROLE_NOTE,A.SUGGESTION,B.TABLE_NAME,CC.INDEXFILE ");
