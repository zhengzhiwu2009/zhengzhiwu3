 ...
 *
 */
public class SeriesCourseAddAction extends MyBaseAction<PeBzzTchCourse>{

	/**
	 * 
	 */
	private static final long serialVersionUID = 7326978188742623099L;
	...
	public void initGrid() {
		...
		this.getGridConfig().addMenuFunction(this.getText("加入推荐系列"), "SeriesAdd_" + this.getSeriesid());
	...
	public Page list(){
		...
		sqlBuffer.append("SELECT COURSE.ID," +
				...
				" (SELECT PK_COURSE_ID FROM RECOMMEND_COURSE WHERE PK_SERIES_ID='" + this.getSeriesid() + "' AND PK_COURSE_ID IS NOT NULL )");
		...
		if (this.getIds() != null && this.getIds().length() > 0) {
			try{
				...
				for(int i = 0 ; i < ids.length; i++){
					PeBzzTchCourse course = this.getGeneralService().getById(ids[i]);
					RecommendCourse rcourse = new RecommendCourse();
					rcourse.setSeries(series);
	...
	public void setServletPath() {
		this.servletPath = "/entity/basic/seriesCourseAdd";
