...
/**
 * 公司内训课程发布Action 2014年10月14日
 * 
 * @author Lzh
 * 
 */
public class PeBzzCourseInternalAction extends MyBaseAction<PeBzzTchCourse> {
	...
	public void initGrid() {
		...
		this.getGridConfig().setTitle("公司内训课程发布");
		...
		this.getGridConfig().addColumn(this.getText("公司编号（一级集体账号）"), "comCode", true, true, true, "");

		this.getGridConfig().addColumn(this.getText("公司名称"), "orgname", true, false, true, "textField");
		// ColumnConfig comCode = new ColumnConfig(this.getText("公司编号(一级集体帐号)"),
		// "comCode", true, true, true, "TextField", false, 100, "");
		// String sql0 = "select p.id from PE_ENTERPRISE p ";
		// comCode.setComboSQL(sql0);
		// this.getGridConfig().addColumn(comCode);
		...
		this.getGridConfig().addColumn(this.getText("课程时长（分钟）"), "courseLen", false, true, true, "TextField", true, 3);
		...
		if (us.getUserLoginType().equals("3")) {
			...
			if (capabilitySet.contains(this.servletPath + "_view.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课程信息"),
								"{return '<a href=\"/entity/teaching/peBzzCourseInternal_courseInfo.action?id='+${value}+'&indexFile='+record.data['indexFile']+'\"  target=\"_blank\">课程信息</a>';}",
								"id");
			}
			...
			if (capabilitySet.contains(this.servletPath + "_setTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("设置讲师图片"), "/entity/teaching/peBzzCourseInternal_setTeacherImg.action", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_removeTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("删除讲师图片"), "/entity/teaching/peBzzCourseInternal_removeImg.action?param=t", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_setSuggest.action")) {
				this.getGridConfig().addMenuFunction(this.getText("设置建议学习人群"), "/entity/teaching/peBzzCourseInternal_setSuggest.action", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_removeSuggest.action")) {
				this.getGridConfig().addMenuFunction(this.getText("删除建议学习人群"), "/entity/teaching/peBzzCourseInternal_removeSuggest.action", false, false);
			}
			...
			if (capabilitySet.contains(this.servletPath + "_impCourse.action") && capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课件文件管理"),
								"{if(record.data['coursewareId'] == ''){return '&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/entity/teaching/peBzzCourseInternal_toUploadCourseware.action?bean.id='+record.data['id']+'\" target=\"_blank\">导入</a>';} else return '<a href=\"#\" onclick=if(true){window.open(\"/entity/manager/course/courseware/filemanager/filemanager_main02.jsp?courseware_id='+record.data['code']+'\")} >管理</a>&nbsp;&nbsp;<a href=\"#\" onclick=if(confirm(\"确定要删除吗\")){window.open(\"/entity/manager/course/courseware/scorm12/delete_scorm_info.jsp?courseware_id='+record.data['code']+'\")} >删除</a>'}",
								"id");
			}
			if (capabilitySet.contains(this.servletPath + "_impCourse.action") && !capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课件文件管理"),
								"{if(record.data['coursewareId'] == ''){return '&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/entity/teaching/peBzzCourseInternal_toUploadCourseware.action?bean.id='+record.data['id']+'\" target=\"_blank\">导入</a>';} else return '<a href=\"#\" onclick=if(confirm(\"确定要删除吗\")){window.open(\"/entity/manager/course/courseware/scorm12/delete_scorm_info.jsp?courseware_id='+record.data['code']+'\")} >删除</a>'}",
								"id");
			}
			if (!capabilitySet.contains(this.servletPath + "_impCourse.action") && capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this.getGridConfig()
			...
			if (capabilitySet.contains(this.servletPath + "_satified.action")) {
				this.getGridConfig().addRenderFunction(this.getText("满意度调查"),
						"<center><a href=\"/entity/teaching/addSatisfactionPaper.action?courseId=${value}&flag=addPaper&area=peBzzCourseInternal\" >添加</a></center>", "id");
			}
	...
	public void setServletPath() {
		// TODO Auto-generated method stub
		this.servletPath = "/entity/teaching/peBzzCourseInternal";
	}
	...
	public Map add() {
		...
		/* 建议学习人群 */
		EnumConst ecFlagSuggestSet = this.getEnumConstService().getByNamespaceCode("FlagFlagSuggestSet", "0");
		...
		/* 课程所属区域-公共区课程 */
		EnumConst ecFlagCoursearea = this.getEnumConstService().getByNamespaceCode("FlagCoursearea", "3");
	...
	public void checkBeforeAdd() throws EntityException {
		String sql = "SELECT p.ID FROM PE_ENTERPRISE p WHERE p.CODE=(SELECT pem.fk_enterprise_id FROM PE_ENTERPRISE_MANAGER PEM WHERE PEM.LOGIN_ID = '"
				+ this.getBean().getComCode() + "') and p.fk_parent_id  is null";
		List comList = this.getGeneralService().getBySQL(sql);
		if (!(null != comList && comList.size() > 0)) {
			throw new EntityException("此一级机构不存在");
		}
	...
	public void checkBeforeUpdate() throws EntityException {
		String sql1 = "SELECT p.ID FROM PE_ENTERPRISE p WHERE p.CODE=(SELECT pem.fk_enterprise_id FROM PE_ENTERPRISE_MANAGER PEM WHERE PEM.LOGIN_ID = '"
				+ this.getBean().getComCode() + "') and p.fk_parent_id  is null";
		List comList = this.getGeneralService().getBySQL(sql1);
		if (!(null != comList && comList.size() > 0)) {
			throw new EntityException("此一级机构不存在");
		}
		String comSql = "SELECT 1 FROM PE_BZZ_TCH_COURSE PBTC WHERE PBTC.ID = '" + this.getBean().getId() + "' AND PBTC.COM_CODE != '" + this.getBean().getComCode()
				+ "' AND PBTC.FLAG_ISVALID = '2'";
		List comCodeList = this.getGeneralService().getBySQL(comSql);
		if (null != comCodeList && comCodeList.size() > 0) {
			throw new EntityException("该课程为发布状态，不允许修改公司编号");
		}
	...
	public Page list() {
		...
		try {
			sqlBuffer
					.append("SELECT * FROM (SELECT A.ID, A.CODE, A.NAME,a.com_code,pe.name orgname , EC1.NAME FLAGCOURSETYPE, EC2.NAME FLAGCOURSECATEGORY, EC3.NAME FLAGCOURSEITEMTYPE, ");
			sqlBuffer.append(" EC4.NAME FLAGCONTENTPROPERTY, EC8.NAME FLAGISVALID, EC9.NAME FLAGOFFLINE, EC5.NAME FLAGISEXAM,  a.course_len,A.TEACHER, ");// EC.NAME
																																							// FLAGISRECOMMEND,
			...
			sqlBuffer.append(" INNER JOIN ENUM_CONST EC ON A.FLAG_ISRECOMMEND = EC.ID");
			...
			sqlBuffer
					.append("     inner JOIN (SELECT p.name, pem.login_id as code  FROM PE_ENTERPRISE p INNER JOIN PE_ENTERPRISE_MANAGER pem on pem.fk_enterprise_id=p.id) PE ON a.com_code = pe.code");// 所在公司
			// lzh
			...
			sqlBuffer.append(" WHERE A.FLAG_COURSEAREA = 'Coursearea3'");
			sqlBuffer.append(" GROUP BY A.ID,A.CODE,A.NAME,EC1.NAME,EC2.NAME,EC3.NAME,EC4.NAME,a.com_code, ");
			sqlBuffer.append(" EC8.NAME,EC9.NAME,EC5.NAME,EC1.NAME,EC2.NAME,EC3.NAME, ");
			sqlBuffer.append(" EC4.NAME,EC8.NAME,EC9.NAME,EC5.NAME,FLAG_ISRECOMMEND,EC.NAME, ");
			sqlBuffer.append(" A.TEACHER,A.STUDYDATES,A.END_DATE,A.ANNOUNCE_DATE, ");
			sqlBuffer.append(" A.ANSWER_BEGINDATE,A.ANSWER_ENDDATE,A.PASSROLE,A.EXAMTIMES_ALLOW,  pe.name,");
			sqlBuffer.append(" EC7.NAME,A.PASSROLE_NOTE,A.SUGGESTION,A.CREATE_DATE,C.COURSE_ID,a.course_len, ");
			...
			do {
				...
				if (!name.equals("enumConstByFlagSuggest.name")) {
					...
					/* 查看一级机构 */
					if (name.equals("comCode")) {
						sqlBuffer.append(" and UPPER(com_code) like UPPER('%" + value + "%')");
		...
		if (this.getIds() != null && this.getIds().length() > 0) {
			...
			try {
				...
				if (action.equals("applyOffline")) {
					for (int i = 0; i < ids.length; i++) {
						String sql = "select * from pe_bzz_tch_course pe where pe.FLAG_ISVALID<>'2' and pe.id='" + ids[i] + "'";
						List list = this.getGeneralService().getBySQL(sql);
						if (!(null == list || list.size() == 0)) {

							map.clear();
							map.put("success", "false");
							map.put("info", msg + "所选课程中有课程未添加满意度调查,请添加满意度调查后再发布");
							return map;
