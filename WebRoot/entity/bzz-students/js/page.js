...
function showPages(name,page,pageCount) { //初始化属性
	this.name = name;      //对象名称
	this.page = page;         //当前页数
...
function showPages(name,startIndex,pageSize,totalCount,form) { //初始化属性
	this.name = name;      //对象名称
	this.startIndex = startIndex; //当前数据数目index
	this.pageSize = pageSize;	//	每页显示条数
	this.totalCount = totalCount;    //总数据条数
	this.pageCount = 1;    //总页数
	this.page = 1;         //当前页数
	this.argName = 'page'; //参数名
	this.showTimes = 1;    //打印次数
	this.form = form;//提交表单form名称
	this.setPageCount();
	this.setPageIndex();
}

showPages.prototype.setPageCount = function() {
    	 if(((this.totalCount + this.pageSize - 1) / this.pageSize)==0)
    		 this.pageCount =  1;
    	 else
    		  this.pageCount =(this.totalCount + this.pageSize - 1) / this.pageSize;
     }

showPages.prototype.setPageIndex = function() {
		this.page = (this.startIndex / this.pageSize) + 1;
     }
	...
	switch (mode) {
		case 0 : //模式1 (页数,首页,前页,后页,尾页)
			...
			if (nextPage > this.pageCount) {
				...
				strHtml += '<span title="Last Page"><a href="javascript:' + this.name + '.toPage(' + this.pageCount + ');">最后一页</a></span>';
			}
			strHtml += '</span>';
			var tempNum = this.startIndex + 10;
			if(tempNum > this.totalCount){
				tempNum = this.totalCount;
			}
			strHtml += '<span class="data_count">显示: ' + (this.totalCount == 0 ? 0 : (this.startIndex + 1)) + '-'+ tempNum + ' of ' + this.totalCount + '</span><br />';
			break;
...
showPages.prototype.getStartIndex = function(page){ //页面跳转
	return (page - 1) * 10;
}
...
showPages.prototype.toPage = function(page){ //页面跳转
	...
	document.forms[0].pageSize.value = this.pageSize;
	document.forms[0].startIndex.value = this.getStartIndex(turnTo);
