	...
	<%
		...
		try {
			...
			if ("teacher".equalsIgnoreCase(userType)) {
				totalItems = peBzzRegService.getOnlineTestCoursesNum(null, title, null, null, null, null, null, null, null, teachclass_id, null);
			} else {
				...
				totalItems = peBzzRegService.getOnlineTestCoursesNum(title, "1", teachclass_id, "student");
			}
			//----------分页开始---------------
			...
			if ("teacher".equalsIgnoreCase(userType))
				testCourseList = peBzzRegService.getOnlineTestCourses(pageover, null, title, null, null, null, null, null, null, null, teachclass_id, null);
			else
				testCourseList = peBzzRegService.getOnlineTestCourses(pageover, title, "1", teachclass_id);
					...
					<td style="padding: 5px 20px 5px 20px;">
						<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								Map scoreM = peBzzRegService.getOnlineTestScore(openCourseId, us.getSsoUser().getId());
							...
							<tr>
								...
								<td class="td">
									...
									<a href="onlinetestcourse_session.jsp?testCourseId=<%=testCourseId%>&isAutoCheck=<%=isAutoCheck%>&isHiddenAnswer=<%=isHiddenAnswer%>&pageInt=<%=pageover.getPageInt()%>">进入</a> &nbsp;&nbsp;&nbsp;
									<%
										Boolean isTrue = peBzzRegService.invalideJudge(testCourseId);
									%>
									<a href="onlinetestcourse_edit.jsp?title=<%=title%>&pageInt=<%=pageInt%>&testCourseId=<%=testCourseId%>">编辑</a> &nbsp;&nbsp;&nbsp;
									<a href="javascript:cfmdel('onlinetestcourse_delexe.jsp?title=<%=title%>&pageInt=<%=pageInt%>&testCourseId=<%=testCourseId%>')" class="button">删除</a>
									<%
