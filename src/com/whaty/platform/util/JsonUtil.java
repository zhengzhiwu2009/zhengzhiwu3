        ...
        for (int i = 0; i < methods.length; i += 1)  {
            try  {
                ...
                if (key.length() > 0 &&
                    ...
                    //增加hibernate一对多、多对一等类型判断，这种类型不进行JSON转换，否则会造成递归死循环
                    if((!useClassConvert && elementObj instanceof Class)|| elementObj instanceof PersistentCollection)
                        continue;
		...
		for (Object object : items) { 
			if(object.getClass().isArray()){
				...
				for (int i = 0; i < columnString.length; i++) {
					String valueString = "";
					if (valueObject[i] instanceof String || valueObject[i] instanceof Character) {
