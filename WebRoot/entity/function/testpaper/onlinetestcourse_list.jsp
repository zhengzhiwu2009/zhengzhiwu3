...
function DetailInfo(testCourseId)
{
	...
	rs = null;
	String courseFLAG=		"select  c.FLAG_ISFREE as isfree, c.FLAG_COURSEAREA as coursearea \n" +
	"  from pe_bzz_tch_course c\n" + 
	" where id = (select FK_COURSE_ID\n" + 
	"               from pr_bzz_tch_opencourse\n" + 
	"              where id = '"+openCourseId+"')";
		MyResultSet  course= db.executeQuery(courseFLAG);
		String isfree = null;
		String coursearea = null;
		if (course.next()) {
			isfree = fixnull(course.getString("isfree"));
			coursearea = fixnull(course.getString("coursearea"));
		}
		db.close(course);
		course = null;
//Lee start 2014年9月14日22:23:46
		...
		try {
			...
			if ("teacher".equalsIgnoreCase(userType)) {
				...
				totalItems = testManage.getOnlineTestCoursesNum(title, "1", teachclass_id, "student");
			}
			//----------分页开始---------------
			...
			if ("teacher".equalsIgnoreCase(userType))
				...
				testCourseList = testManage.getOnlineTestCourses(pageover, title, "1", teachclass_id);
					...
					<td style="padding: 5px 20px 5px 20px;">
						<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								<td class="td">
								<%
								if("40288a7b39968644013996bf01e7004b".equals(isfree)){//‘40288a7b39968644013996bf01e7004b’ 表示免费课程
								%>
									-
								<%
								}else {
								...
								<td class="td">
									<%=(null == examTimes || "".equals(examTimes)) ? "0" : examTimes%>
								</td>
								<td class="td">
									<%=ispass%>
								</td>
								...
								<td class="td">
									...
									%>
									<a href="onlinetestcourse_session.jsp?pbtc=<%=teachclass_id %>&testCourseId=<%=testCourseId%>&isAutoCheck=<%=isAutoCheck%>&isHiddenAnswer=<%=isHiddenAnswer%>&pageInt=<%=pageover.getPageInt()%>">进入</a> &nbsp;&nbsp;&nbsp;
									...
									<%
										if (score >= passScore) {
											...
											if(!"40288a7b39968644013996bf01e7004b".equals(isfree)){//‘40288a7b39968644013996bf01e7004b’表示免费课程
												out.print("<font color=red>未通过，课后测验次数达到上限！</font>");
											}else{
												%>
									...
									<script type="text/javascript">
															function jinruceyan(){
																...
																%>
																	...
																	window.open('onlinetestcourse_session.jsp?pbtc=<%=teachclass_id %>&testCourseId=<%=testCourseId%>&isAutoCheck=<%=isAutoCheck%>&isHiddenAnswer=false&pageInt=<%=pageover.getPageInt()%>', 'coursetest', '');
																}else{
																	return false;
																}
															}
												...
												if (!"false".equalsIgnoreCase(flag)) {//正常，可进行课后测验
									%>
									<a href="javascript:void(0);" onclick="jinruceyan()" style="color: red;"><b>考试</b></a>
									<script type="text/javascript">
															function jinruceyan(){
									...
									<%
										if (score >= passScore) {
																...
																if(!"40288a7b39968644013996bf01e7004b".equals(isfree)){//‘40288a7b39968644013996bf01e7004b’表示免费课程
																	...
																	%>
																	<a href="javascript:void(0);" onclick="jinruceyan()" style="color: red;"><b>考试3</b></a>
														<script type="text/javascript">
																				function jinruceyan(){
																					if(confirm('确定要进入课后测验吗')){
																					<%
									...
									<script type="text/javascript">
															function jinruceyan(){
																...
																<%
																	...
																	//Lee end
																%>
																	window.open('onlinetestcourse_session.jsp?pbtc=<%=teachclass_id %>&testCourseId=<%=testCourseId%>&isAutoCheck=<%=isAutoCheck%>&isHiddenAnswer=<%=isHiddenAnswer%>&pageInt=<%=pageover.getPageInt()%>','coursetest', '');
																}else{
																	return false;
																}
															}
