...
<%@ page import="com.whaty.platform.entity.basic.*,com.whaty.platform.entity.config.*,net.sf.ehcache.CacheManager"%>
...
<%@ page import="com.whaty.platform.test.TestManage,com.whaty.util.HzphCache" %>
...
<%
	...
	//url=url+"WEB-INF\\classes\\ehcache.xml";
	int flag=0;
	...
	idx = 1;
  	for(Iterator it = set.iterator();it.hasNext();)
  	{
		...
		for(int i=0;i<questionList.size();i++,idx++)
	    {
			...
			if(question.getStatus().equals("new"))
			{
				...
				flag=testManage.addPaperQuestion(question);
			}
			else
			{
				question.setTestPaperId(paperId);
  	...
  	//清空缓存
  	//	if(flag>0){
  	//	HzphCache hzphCache=new HzphCache();
	//  CacheManager ehCacheManager=new CacheManager();
	//	hzphCache.setEhCacheManager(ehCacheManager);
	//	hzphCache.setCacheName("hzphCache");
	//	hzphCache.remove("onlineExam"+paperId);
   //}
%>
<script>
