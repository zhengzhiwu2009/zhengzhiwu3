...
function SaveSCODataToPlatform(dmeKey, dmeValue,jessonid){
	...
	var _command = "?d="+d+"&command=putdata&"+jessonid//+"&param="+dmeID+"&param="+dmeValue;
     ...
     {
		_command += "&dmeID=" + dmeKey[i] + "&dmeValue=" + encodeURIComponent(dmeValue[i]);
     }
     createRequest();
     dataUrl = servleturl + _command;
     try{
     	saveData(dataUrl);
     }catch(e){
     	return "failed";
...
function saveDataResult(){
	 if (request.readyState == 4)
       if (request.status == 200){
       		 ...
       		 var as ;
       		 if(s>0) {
       			 as = text.substring(s+1);
       			 if(as!=d) {
       				 if(times<=3 ){
       					 saveData(dataUrl+"&d2="+Math.random());
       				 }
       				 if(times>3){
       				 }
       			 }
...
//out param
var servleturl = "";
//apiurl 为LMSFrmae2.jsp页面中的全局变量,用于配置js版servlet的地址;
...
APIAdapter = {
	...
	servletURL:null,
	theSCOData:null,
    setSCOID:function(scoID)
	{
		window.parent.whatyExtAPI.setSCOID(scoID);
	},
	init : function(){
		if (this._Debug)
      {
   ...
   LMSFinish : function(param)
   {
      ...
      if ( (tempParm == "null") || (tempParm == "") )
      {
         if ( this.CheckInitialization() == true )
         {
            ...
            var sessionTime = window.parent.computeTime();//get the platform computed time.
            if(sessionTime == null || sessionTime =='' ){
            	sessionTime = this.theSCOData.getValue("cmi.core.session_time");
            }
            this.theSCOData.setValue("cmi.core.session_time",sessionTime);
			if (sessionTime != null) 
			{
				//添加对时间NaN的处理；
				...
				var tem = total_time; // 临时的total_time;
