...
public class UserPriorityInterceptor extends MethodFilterInterceptor {
	...
	protected String doIntercept(ActionInvocation invocation) throws Exception{
		...
		if (wc == null) {
			...
			SingleLogin singleLotin = new SingleLogin();
			singleLotin.isAlreadyEnter(us.getLoginId());
			if(namespace.indexOf("workspace")>=0||actionName.indexOf("onlinePayment")>=0){
				havePermission = true;
			}else{
			...
			if (havePermission) {
				...
				//记录日志 (如果参数不是请求 _excute , _abstractList),新加是否记录日志的配置
				if(method!=null && !method.equals("_execute") && !method.equals("_abstractList") && ((MyBaseAction)invocation.getAction()).isDoLog()){
					...
					if(json!=null){
						if(json.indexOf("\"success\":\"true\"")>0){
							...
							log.setNotes(req_parm.toString());
						...
						//非空判断
						if(null != result && result.equals("msg")){
