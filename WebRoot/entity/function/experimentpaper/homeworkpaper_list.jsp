...
try
{
	...
	if("teacher".equalsIgnoreCase(userType))
		totalItems = testManage.getExperimentPapersNum(null,null,null,null,null,null,null,null,null,null,teachclass_id);
	else
		totalItems = testManage.getActiveExperimentPapersNum(null,null,null,null,null,null,null,null,null,null,teachclass_id);
	//----------分页开始---------------
	...
	if("teacher".equalsIgnoreCase(userType))
		testPaperList = testManage.getExperimentPapers(pageover,null,null,null,null,null,null,null,null,null,null,teachclass_id);
	else
		testPaperList = testManage.getActiveExperimentPapers(pageover,null,null,null,null,null,null,null,null,null,null,teachclass_id);
%>
...
<%
	...
	{
%>	
                            <td align="center"><a href="homeworkpaper_add.jsp" class="tj">[添加新实验]</a>&nbsp;</td>
                            <!-- td align="center"><a href="paperpolicy_add.jsp" class="tj">[添加新实验策略]</a>&nbsp;</td>
                            <td align="center"><a href="homeworkpape实验olicy_list.jsp?type=list" class="tj">[查看实验策略]</a>&nbsp;</td-->
<%
	}
%>	                            	
                            <!--  td align="center"><a href="homeworkhistory_list.jsp" class="tj">[查看已交实验]</a></td-->
                            <td>
                            	<form method="pos实验 action="homeworkpaper_list.jsp" name="paper_listSearchForm"><!--
                 ...
                 <tr>
                   <td height="26" background="../images/tabletop.gif" style="background-repeat: no-repeat;filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true, sizingMethod=scale, src='../images/tabletop.gif');">
                    ...
                    <tr>
					  ...
					  <td width="1%" align="center" valign="bottom"><img src="images/topxb.gif"></td>
                      <td width="22%" align="center" class="title">实验时间</td>
                      <td width="1%" align="center" valign="bottom"><img src="images/topxb.gif"></td>
                  <%
					if("teacher".equalsIgnoreCase(userType))
					{
                    	...
                    	<%                         
                          	...
                          	while(it.hasNext()){
								//根据奇偶行，决定每行的背景颜色          	
                             	...
                             	ExperimentPaper homeworkPaper = (ExperimentPaper)it.next();
						...
						<table width="99%" border="0" cellspacing="0" cellpadding="0" class="mc1">
                          <tr bgcolor="<%=trBgcolor%>"> 
                           	 ...
                           	 else
                            {
                            	...
                            	int homeWorkNum = testManage.getExperimentPaperHistorysNum(user_id,paperId,null,null,teachclass_id);
                             %>
                             <td width="30%" align="center"  class="td1">
                             <%
                             	if(testManage.getExperimentPaperExpired(paperId))
                             	{
                              %>
                             <%if(homeWorkNum>0)  out.print("状态：已交"); else out.print("<font color=red>状态：未交</font>");%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 现在不是交实验时间
                              <%
                              	}
                              	else
                              	{
                               %>
                               <%if(homeWorkNum>0) {%>状态：已交&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="homeworkhistory_list.jsp?paperId=<%=paperId%>&prepageInt=<%=pageInt %>">查看已交实验</a>
                               <% }else{%><font color=red>状态：未交</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                          <%
		if(mock_login == null || !mock_login.equals("1")){
		%>
                            <a href="#" onclick="javascript:window.open('homeworkpaper_pre.jsp?id=<%=paperId%>&pageInt=<%=pageover.getPageInt() %>','sy','resizable,width=800,height=600')">做实验</a>
...
<%
	...
	if("teacher".equalsIgnoreCase(userType))
	{%>							<a href="homeworkhistory_list.jsp?paperId=<%=paperId%>&prepageInt=<%=pageInt %>">已交实验列表</a>
