...
<%
	...
	Object passStudy = session.getAttribute("passStudy");
...
function toTest(){
	if(confirm('您已经完成本课程的课件学习，现在可以进行本课程的课后测验了，\n\n必须通过课后测验才能够获取学时，是否现在进入？')){
		//alert('您现在可以进行本课程的考试了。');
		var toUrl = '/entity/function/testpaper/onlinetestcourse_list.jsp?opencourseId=<%=opencourseId%>';
		//window.force.open(toUrl);
		document.getElementById('mainer').src=toUrl;
		//window.location=toUrl;
	}else{
		alert('您选择本次不进行课后测验，下次进入本课程会再次提醒您！\n必须通过课后测验才能够获取学时，请牢记！');
	}
}
...
function openCourseWare(){
	...
	windows['courseWin'] = window.open('/sso/bzzinteraction_training.action?course_id=<%=courseId%>&opencourseId=<%=opencourseId%>&path=<%=path%>', 'courseWin', 'height=680,width=1050,top=0,left=50,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
	}
											 ...
											 %>
												<tr>
													<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
														<a href="/entity/function/course_state.jsp?open_course_id=<%=teachclass_id%>&exam_y_n=<%=exam_y_n %>&courseCode=<%=openCourse.getBzzCourse().getCode() %>&opencourseId=<%=opencourseId %>" target="mainer" class="menuzi">课程状态</a>
													</td>
												</tr>
												<tr>
													<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
												...
												<tr>
													...
													<%}else{ %>
														...
														<td width="138" height="45" align="center" valign="top" background="images/menu8_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
															<a style="cursor: pointer;" onclick="alert('该课程通过后,不再提供访问.');" class="menuzi"><span id="kjxi">课件学习</span> </a>
														</td>
													<%} %>
												</tr>
												<tr>
												...
												<%
													//已经学完并且课程有满意度ID
													if (studentOperationManage.isLearningCompleted(opencourseId, userId) && voteId != null && !"".equals(voteId)) {
														//是否存在成绩，如果成绩为空则显示满意度，如果不为空则可直接进入考试
														if (studentOperationManage.isShowManyi(userId, openCourse.getId())) {
															//课程需要考试
															if (openCourse.getBzzCourse().getEnumConstByFlagIsExam() != null && "1".equalsIgnoreCase(openCourse.getBzzCourse().getEnumConstByFlagIsExam().getCode())) {
																...
																Map scoreM = testManage.getOnlineTestScore(opencourseId, student.getId());
												...
												<tr>
													<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
														<a href="/entity/function/manyidu_list.jsp?opencourseId=<%=opencourseId%>" target="mainer" class="menuzi">满意度调查</a>
													</td>
												</tr>
												<%
													//不及格或未考试直接弹出继续考试提示
												...
												<%
															...
															//没有填写过满意度
															if (!studentOperationManage.isManyiCompleted(openCourse.getBzzCourse().getId(), userId)) {
												%>
												<tr>
													<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
														<a href="/entity/function/manyidu_list.jsp?opencourseId=<%=opencourseId%>" target="mainer" class="menuzi">满意度调查</a>
												...
												<%
													//填写过满意度并且课程需要考试
													} else if (studentOperationManage.isManyiCompleted(openCourse.getBzzCourse().getId(), userId) && openCourse.getBzzCourse().getEnumConstByFlagIsExam() != null
												...
												<%
													...
													<tr>
														<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
															<a href="/entity/function/manyidu_list.jsp?opencourseId=<%=opencourseId%>&flag=false" target="mainer" class="menuzi">满意度调查</a>
														</td>
													</tr><%
															}
														}
												...
												<%
													...
													<tr>
														<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
															<a href="/entity/function/testpaper/onlinetestcourse_list.jsp?opencourseId=<%=opencourseId%>&flag=<%=exam_flag %>" target="mainer" class="menuzi">课后测验</a>
														</td>
													</tr>
													<%
													}
												...
												<tr>
													<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
														<a href="/entity/function/answer/index.jsp" target="mainer" class="menuzi">课程答疑</a>
													</td>
												</tr>
												<%
									              	} 
