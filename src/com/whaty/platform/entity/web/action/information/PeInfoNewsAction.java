...
//		this.getGridConfig().addColumn(this.getText("新闻类型"),"peInfoNewsType.name");
		ColumnConfig column = new ColumnConfig(this.getText("新闻类型"), "peInfoNewsType.name", true, true, true, "TextField", false, 50, "");
		column.setComboSQL("select pnt.id,pnt.name from pe_info_news_type pnt");
	...
	public void checkBeforeAdd() throws EntityException{
		...
		this.getBean().setIsNew("0");
		String summary=this.getBean().getSummary();
		if(summary==null || summary.equals("") || summary.length()<=0)
		{
			this.getBean().setSummary("暂无");
		}
	}
	...
	public DetachedCriteria initDetachedCriteria() {
		...
		if(!"reportDate".equals(this.getSort())) {
