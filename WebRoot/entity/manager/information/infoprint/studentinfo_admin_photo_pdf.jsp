...
<%
	...
	if (!batch_id.equals("")) {
		sql_batch= "and peb.id = '" + batch_id + "'";
	}

	String esql = "";
	if(erji_id == null || "".equals(erji_id)) {
		esql = "select name from pe_enterprise p  where p.id = '"+enterprise_id+"'";
	} else {
		esql = "select pp.name||'——'||p.name as name from pe_enterprise p,pe_enterprise pp where p.fk_parent_id=pp.id and p.id='"+erji_id+"'";
	}
	...
	Font fontDWMC = new Font(bfChinese, 12, Font.NORMAL);
	...
	try {
		...
		String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+File.separator;
		...
		String sql_t = "select rownum , ps.true_name as name  , ps.reg_no as regno ,  ss.password as passwords ,\n"
			...
			+ " ps.email as email,pb.batch_code as batchCode,ps.photo,ee.yijiCode as yijiCode  from pe_bzz_student ps inner join sso_user ss on ps.fk_sso_user_id = ss.id and ss.flag_isvalid='2' and ps.flag_rank_state='402880f827f5b99b0127f5bdadc70006' \n"
			+ " inner join (select be.student_id from  pe_bzz_examscore be,pe_bzz_exambatch peb where peb.id=be.exambatch_id and be.status='402880a9aaadc115061dadfcf26b0003' "+sql_batch+") bb on bb.student_id=ps.id "
			+ " inner join pe_bzz_batch pb on ps.fk_batch_id = pb.id  "
			+ " inner join (select pe.id,pe.name,pes.name as ename,pe.fk_parent_id as fk_parent_id ,pes.code as yijiCode from pe_enterprise pe\n"
		...
		String sql_conut = "select count(*) from pe_bzz_student ps inner join sso_user ss on ps.fk_sso_user_id = ss.id and ss.flag_isvalid='2' and ps.flag_rank_state='402880f827f5b99b0127f5bdadc70006' \n"
			+ " inner join (select be.student_id from  pe_bzz_examscore be,pe_bzz_exambatch peb where peb.id=be.exambatch_id and be.status='402880a9aaadc115061dadfcf26b0003' "+sql_batch+") bb on bb.student_id=ps.id "
			+ " inner join pe_bzz_batch pb on ps.fk_batch_id = pb.id  "
			+ " inner join (select pe.id,pe.name,pes.name as ename,pe.fk_parent_id as fk_parent_id ,pes.code as yijiCode from pe_enterprise pe\n"
			+ " left outer join pe_enterprise pes on pe.fk_parent_id= pes.id) ee\n"
		...
		int headerwidths[] = { 21, 4, 21, 4, 21, 4,21, 0};
	 	for(l = 0 ;l<num;l++){
	 		if(rs!=null&&rs.next()){
	 		username = rs.getString("name");
	 		regNo=rs.getString("regno");
	 		batchCode=fixnull(rs.getString("batchCode"));
	 		yijiCode=fixnull(rs.getString("yijiCode"));
	 		photo=fixnull(rs.getString("photo"));
			rownum = rs.getInt("rownum");
			
			if(photo!=null && !photo.equals(""))
				tmppath=basePath+"incoming"+File.separator+"student-photo"+File.separator+batchCode+File.separator+yijiCode+File.separator+photo;
			else
				tmppath=basePath+"incoming"+File.separator+"student-photo"+File.separator+"noImage.jpg";
		...
		//cell = new PdfPCell(new Phrase("照片:"+tmppath,fontDWMC));
		//cell.setBorder(1);
		//cell.setMinimumHeight(150);
		//pdfTable.addCell(cell);
		
		Image image = Image.getInstance(tmppath);
		...
		cell = new PdfPCell(new Phrase("学 号:"+regNo,fontDWMC));
		cell.cloneNonPositionParameters(border);
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		nest.addCell(cell);
