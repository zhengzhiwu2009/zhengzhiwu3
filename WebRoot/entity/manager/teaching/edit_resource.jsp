...
<html>
	...
	<script type="text/javascript">
		function doSubmit() {
			...
			document.myForm.action = "peResource_editPeResource.action";
	...
	<body>
		...
		<form action="" name="myForm" method="post" enctype="multipart/form-data">
			...
			<input type="hidden" name="id" id="id" value="<s:property value="peResource.id"/>"/>
			...
			<table cellspacing="0" cellpadding="0">
					<tr>
						...
						<td>
							<input type="text" size="50" name="name" id="name" value="<s:property value="peResource.name"/>">
						</td>
					</tr>
					<tr>
						<th>
						...
						<td>
							<!--  
							<input type="text" size="50" name="reseType" id="resetype" value="<s:property value="peResource.resetype"/>">
							-->
							<select id="reseType" name="reseType">
								<s:iterator value="resourceTypeList" id="type">
									<option value="<s:property value="#type.id"/>" <s:if test="peResource.enumConstByFlagResourceType.id == #type.id">selected=selected</s:if>><s:property value="#type.name"/></option>
								</s:iterator>
							</select>
						</td>
					</tr>
						...
						<td>
							<s:if test="resourceTagList != null && resourceTagList.size() > 0">
								<s:iterator value="resourceTagList" id="tag">
									<input type="checkbox" name="resourceTag" value="<s:property value="#tag.id"/>" <s:if test="(peResource.resourceTagIds).indexOf(#tag.id) >= 0">checked=checked</s:if>/><s:property value="#tag.name"/>
								</s:iterator>
							</s:if>
						</td>
					</tr>
						...
						<td>
							<select name="show">
								<s:iterator value="isValidList" id="show">
									<option value="<s:property value="#show.id"/>" <s:if test="peResource.enumConstByFlagIsvalidSPshouye.id == #show.id">selected</s:if>>
						...
						<td>
							<select name="down">
								<s:iterator value="isValidList" id="down">
									<option value="<s:property value="#down.id"/>" <s:if test="peResource.enumConstByFlagIsvalidSPxianzai.id == #down.id">selected</s:if>>
					...
					<tr>
						...
						<td>
							<input type="text" style="width: 80px" name="replyDate" id="replyDate" value="<s:date name="peResource.replyDate" format="yyyy-MM-dd"/>" />
					...
					<tr>
						...
						<td>
							<input type="text" size="50" id="fabuunit" name="fabuunit" value="<s:property value="peResource.fabuunit"/>"/>
						</td>
					</tr>
					<tr>
						<th align="right">
						...
						<td>
							<input type="checkbox" name="showuser" id="showuser" value="jgmanager" <s:if test="(peResource.showUsers).indexOf('jgmanager') >= 0">checked=checked</s:if>/>监管集体
							<input type="checkbox" name="showuser" id="showuser" value="jgstudent" <s:if test="(peResource.showUsers).indexOf('jgstudent') >= 0">checked=checked</s:if>/>监管学员
							<input type="checkbox" name="showuser" id="showuser" value="jtmanager" <s:if test="(peResource.showUsers).indexOf('jtmanager') >= 0">checked=checked</s:if>/>集体
							<input type="checkbox" name="showuser" id="showuser" value="jtstudent" <s:if test="(peResource.showUsers).indexOf('jtstudent') >= 0">checked=checked</s:if>/>学员
						</td>
					</tr>
					<tr>
						<th>
						...
						<td>
							<textarea rows="6" cols="70" id="describe" name="describe"><s:property value="peResource.describe"/></textarea>
						</td>
					</tr>
					<tr>
						<th>
