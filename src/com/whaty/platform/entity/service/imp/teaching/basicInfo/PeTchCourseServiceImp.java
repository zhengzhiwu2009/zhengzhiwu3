...
public class PeTchCourseServiceImp implements PeTchCourseService {
	...
	public int save_uploadCourse(File file) throws EntityException {
		...
		for(int i=1; i<rows; i++) {
			try {
				...
				instance.setTime( String.valueOf(Long.parseLong(temp)));
	...
	public PeBzzTchCourseware savePeBzzTchCourseware(PeBzzTchCourseware peBzzTchCourseware) throws EntityException{
		try {
/*	更新课程信息时会删掉课件信息，暂时屏蔽掉 zhangchengli 2012-12-13
 * 	String sql = "delete from pe_bzz_tch_courseware c where c.name = '"+peBzzTchCourseware.getName()+"'";
		this.generalDao.executeBySQL(sql);
*/		
			peBzzTchCourseware = (PeBzzTchCourseware)this.getGeneralDao().save(peBzzTchCourseware);
		} catch (Exception e) {
			e.printStackTrace();
			throw new EntityException("保存课件失败");
		}
		return peBzzTchCourseware;
	}

	@Override
	public PrBzzTchOpencourse add(PrBzzTchOpencourse prBzzTchOpencourse)
			throws EntityException {
		this.getGeneralDao().save(prBzzTchOpencourse.getPeBzzTchCourse());
		prBzzTchOpencourse = (PrBzzTchOpencourse) this.getGeneralDao().save(prBzzTchOpencourse);
...
//		dc.createAlias("peBzzTchCourse", "peBzzTchCourse");
		dc.add(Restrictions.in("peBzzTchCourse.id", ids));
		List idList= this.getGeneralDao().getDetachList(dc);
		//先删除默认开课记录
		String ids_id = "";
		for(Object o : idList) {
			ids_id += "'"+o.toString()+"',";
		}
		ids_id = ids_id.substring(0, ids_id.length()-1);
		String sql_sql = "select * from STUTTIME where FK_OPEN_COURSE_ID in ("+ids_id+")";
		List list = this.getGeneralDao().getBySQL(sql_sql);
		if(list.size()>0) {
			String sql = "delete from STUTTIME where FK_OPEN_COURSE_ID in (" +ids_id+")";
			this.getGeneralDao().executeBySQL(sql);
		}
		this.getGeneralDao().deleteByIds(PrBzzTchOpencourse.class, idList);	
		//删除课程
		int i = this.getGeneralDao().deleteByIds(PeBzzTchCourse.class, ids);
		return i;
	}

	@Override
	public PeBzzTchCourse saveCourseAndCourseware(PeBzzTchCourse dbInstance,
			PeBzzTchCourse bean) {
		if(bean.getName()==null || bean.getCode() ==null
				|| dbInstance.getName()==null ||dbInstance.getCode() ==null
				|| !bean.getName().equals(dbInstance.getName())
				|| !bean.getCode().equals(dbInstance.getCode())){//当name和code有修改或者有空值时候才会更新课件内容
			...
			dCourseware.add(Restrictions.eq("peBzzTchCourse.id", dbInstance.getId()));
			List coursewareList = generalDao.getList(dCourseware);
			//修改课件信息
			PeBzzTchCourseware peBzzTchCourseware = null;
			if (coursewareList != null && coursewareList.size() > 0) {
				peBzzTchCourseware = (PeBzzTchCourseware) coursewareList.get(0);
				peBzzTchCourseware.setName(bean.getName());
				peBzzTchCourseware.setCode(bean.getCode());
				peBzzTchCourseware =(PeBzzTchCourseware) generalDao.save(peBzzTchCourseware);
			} 
			//修改scorm信息
			String scormCourseInfoSql = 
				"update scorm_course_info sci set sci.id='"+bean.getCode()
					+"',sci.title='"+bean.getName()
					+"' where sci.id='"+dbInstance.getCode()+"'";
			int r = generalDao.executeBySQL(scormCourseInfoSql);
		}
		bean = (PeBzzTchCourse) generalDao.save(bean);
		return bean;
	}
}
