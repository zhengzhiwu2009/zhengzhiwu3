...
<table border=0 cellpadding=0 cellspacing=0 width=4081 style='border-collapse:
 ...
 <tr height=18 style='height:13.5pt'>
  ...
  <td class=xl637318 width=92 style='width:69pt'>本次新增预报人数</td>
  <td class=xl637318 width=64 style='width:48pt'>本次新增沟通记录</td>
  <td class=xl637318 width=38 style='width:29pt'>本次备注</td>
 </tr>
 <%
				sql = "select a.id,a.code,a.name,b.num09,c.num10,d.signnum,e.paynum,f.m_name,f.phone,f.mobile_phone,f.login_num from\n" +
				"(select e.id,e.code,e.name from pe_enterprise e where e.fk_parent_id is null order by e.code) a,\n" + 
				...
				while(rs!=null&&rs.next())
				{
					...
					String erji_sql =  "select id,code,name,num09,num10,nvl(num09,0)+nvl(num10,0) as num_total,signnum,paynum,num09exam,num09late,\n" +
						"replace(to_char(nvl(num09tongguo,0)*100/nvl(num09shikao,1),'990.9')||'%',' ','') as tongguo_rate,\n" + 
						"replace(to_char(nvl(num09youxiu,0)*100/nvl(num09shikao,1),'990.9')||'%',' ','') as youxiu_rate from\n" + 
						"(select a.id,a.code,a.name,b.num09,c.num10,d.signnum,e.paynum,g.num09exam,h.num09late,i.num09shikao,j.num09youxiu,k.num09tongguo \n" + 
						" from\n" + 
						"(select e.id,e.code,e.name from pe_enterprise e where e.fk_parent_id='"+id+"' order by e.code) a,\n" + 
						...
						"where a.id=b.id(+) and a.id=c.id(+) and a.id=d.id(+) and a.id=e.id(+) and a.id=g.id(+) and a.id=h.id(+) and a.id=i.id(+) and a.id=j.id(+) and a.id=k.id(+)\n" + 
						" and (num09 is not null or num10 is not null or signnum is not null)) order by code";
...
%>
 <tr class=xl647318 height=18 style='height:13.5pt'>
  ...
  <td class=xl667318 style='border-left:none'><%=ej_code %></td>
