...
public class ExpressionParse implements Serializable {
				...
				if(temp == null ){
					...
					if(stack.isEmpty()){
						...
						if(clazz.getName().equals("java.lang.String")){
							//Lee start 2014年1月7日 判断查询条件是否为系统编号[regNo] 如果是则忽略大小写
							if(propertyName.toLowerCase().indexOf("regno") == -1){
								criterion = Restrictions.like(propertyName, temp.toString(),MatchMode.ANYWHERE);//Lee 原版
							}else{
								if("regno".equals(propertyName.toLowerCase())){
									criterion = Restrictions.like(propertyName, temp.toString(),MatchMode.ANYWHERE).ignoreCase();	
								}else if(propertyName.indexOf(".") != -1){
									if("regno".equals(propertyName.toLowerCase().substring(propertyName.indexOf(".") + 1))){
										criterion = Restrictions.like(propertyName, temp.toString(),MatchMode.ANYWHERE).ignoreCase();	
									}else{
							...
							cl.setTimeInMillis(startDate.getTime()+24*60*60*1000);
							criterion = Restrictions.and(Restrictions.ge(propertyName, temp),Restrictions.lt(propertyName, cl.getTime()));
						}else{						
...
//		System.out.println(o.toString());
	...
	public static List strToList(String exp,Class clazz) throws Exception{
		...
		boolean b = false;
		if(exp.trim().equals("!null")){
			 list.add(exp);
			 return list;
		}else if(exp.trim().equals("null")||exp.trim().equals("=null")){
			list.add(null);
			return list;
		}
