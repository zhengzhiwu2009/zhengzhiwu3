...
public class PeBzzCourseAssManagerAction extends MyBaseAction<PeBzzTchCourse> {
	...
	public void initGrid() {
		...
		this.getGridConfig().setTitle("协会内训课程发布");
		...
		if (us.getUserLoginType().equals("3")) {
			...
			if (capabilitySet.contains(this.servletPath + "_view.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课程信息"),
								"{return '<a href=\"/entity/teaching/peBzzCourseAssManager_courseInfo.action?id='+${value}+'&indexFile='+record.data['indexFile']+'\"  target=\"_blank\">课程信息</a>';}",
								"id");
			}
			...
			if (capabilitySet.contains(this.servletPath + "_setTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("设置讲师图片"), "/entity/teaching/peBzzCourseAssManager_setTeacherImg.action", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_removeTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("删除讲师图片"), "/entity/teaching/peBzzCourseAssManager_removeImg.action?param=t", false, false);
			}
			...
			if (capabilitySet.contains(this.servletPath + "_satified.action")) {
				this.getGridConfig().addRenderFunction(this.getText("满意度调查"),
						"<center><a href=\"/entity/teaching/addSatisfactionPaper.action?courseId=${value}&flag=addPaper&area=peBzzCourseAssManager\" >添加</a></center>", "id");
			}
	...
	public void setServletPath() {
		// TODO Auto-generated method stub
		this.servletPath = "/entity/teaching/peBzzCourseAssManager";
	}
	...
	public Map add() {
		...
		/* 课程所属区域-协会内训课程 */
		EnumConst ecFlagCoursearea = this.getEnumConstService().getByNamespaceCode("FlagCoursearea", "4");
	...
	public Page list() {
		...
		try {
			...
			sqlBuffer.append(" WHERE A.FLAG_COURSEAREA = 'Coursearea4'");
