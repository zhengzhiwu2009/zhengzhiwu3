	...
	if(us.getRoleId().equals("2") || us.getRoleId().equals("402880a92137be1c012137db62100006"))  //表示为一级主管和一级辅导员
	{
		...
		sql_ent="("+sql_entbk.substring(0,sql_entbk.length()-1)+")";
				...
				String sql = "select a.id,a.name,a.total_num,nvl(b.completed_num,0) as completed_num,(a.total_num-nvl(b.completed_num,0)) as incompleted_num from "
							+"	(select b.id,b.name,count(s.id) as total_num from (pe_bzz_batch b left outer join pe_bzz_student s on s.fk_batch_id=b.id and s.flag_rank_state='402880f827f5b99b0127f5bdadc70006') left outer join sso_user u on s.fk_sso_user_id=u.id and u.flag_isvalid='2' where 1=1 " + sql_con1 + " group by b.id,b.name) a,"
							...
							+"		where b.id=s.fk_batch_id(+) and e.id = s.fk_enterprise_id and b.id=o.fk_batch_id and s.fk_sso_user_id=u.id and u.flag_isvalid='2' and s.flag_rank_state='402880f827f5b99b0127f5bdadc70006' and u.id=cs.student_id and cs.course_id=o.id and o.fk_course_id=c.id and cs.learn_status='COMPLETED'  "+sql_con+" group by b.id,b.name,s.id,s.reg_no,s.name) "
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=811 style='border-collapse:
 ...
 <tr height=40 style='mso-height-source:userset;height:30.0pt'>
  ...
  <td class=xl24 style='border-top:none;border-left:none'>完成基础课学时人数</td>
  ...
  <td class=xl24 style='border-top:none;border-left:none'>未学完基础课人数</td>
 </tr>
 <%
 ...
 <tr height=26 style='mso-height-source:userset;height:19.5pt'>
  ...
  <td class=xl26 style='border-top:none;border-left:none' x:num><%= total_num%>　</td>
  <td class=xl26 style='border-top:none;border-left:none' x:num><%=completed_num %>　</td>
  <td class=xl26 style='border-top:none;border-left:none' x:num>0</td>
  <td class=xl26 style='border-top:none;border-left:none' x:num>0</td>
  <td class=xl26 style='border-top:none;border-left:none' x:num><%=incompleted_num %>　</td>
 </tr>
 <%    }
       db.close(rs); %>
