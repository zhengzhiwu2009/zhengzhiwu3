...
public class BzzAssessAction extends MyBaseAction {
	...
	public String updateAssess() {
		...
		try {
			PeBzzSuggestion peBzzSuggestion = this.bzzAssessService.getPeBzzSuggestion(sugid);
			peBzzSuggestion.setDataDate(new Date());
			peBzzSuggestion.setContent(suggestion);
			//System.out.println("yaoqiu============>"+yaoqiu);
			peBzzSuggestion.setYaoQiu(yaoqiu);
			this.bzzAssessService.updatepeBzzSuggestion(peBzzSuggestion);
			
			
			DetachedCriteria tedc = DetachedCriteria.forClass(PeBzzAssess.class);
			DetachedCriteria teccdc = tedc.createCriteria("peBzzCourseFeedback","peBzzCourseFeedback");
			...
			teccdc.add(Restrictions.sqlRestriction("1=1 order by to_number(px)"));
	...
	public String toAssess() {
		...
		try {
			...
			dc.add(Restrictions.eq("ssoUser.id", ssoUser.getId()));
			
			List<PeBzzStudent> list = this.getGeneralService().getList(dc);
			
			if(list.size() > 0) {
				Date startDate = list.get(0).getPeBzzBatch().getEvalStartDate();
				Date endDate = list.get(0).getPeBzzBatch().getEvalEndDate();
				Calendar cal = Calendar.getInstance();
				cal.set(Calendar.HOUR_OF_DAY, 0);
				cal.set(Calendar.MINUTE, 0);
				cal.set(Calendar.SECOND, 0);
				if(startDate.after(cal.getTime())) {
					ServletActionContext.getRequest().setAttribute("errorMessage", "评估未开始!!");
					return "AccessError";
				}
				if(endDate.before(cal.getTime())) {
					ServletActionContext.getRequest().setAttribute("errorMessage", "评估已结束!!");
					return "AccessError";
				}
			}
		} catch (EntityException e) {
	...
	public void setServletPath() {
		this.servletPath = "/entity/workspaceStudent/bzzAssess";
	}
