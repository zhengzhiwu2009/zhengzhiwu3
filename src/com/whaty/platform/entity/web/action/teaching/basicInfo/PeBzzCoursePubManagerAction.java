...
/**
 * 专项课程发布Action 2014年6月24日
 * 
 * @author Lee
 * 
 */
public class PeBzzCoursePubManagerAction extends MyBaseAction<PeBzzTchCourse> {
	...
	public void initGrid() {
		...
		this.getGridConfig().setTitle("公共课程发布");
		...
		ColumnConfig columnConfigPreTest = new ColumnConfig(this.getText("是否课前测试"), "enumConstByFlagPreTest.name", true, true, true, "TextField", false, 100, "");
		String pretestsql = "select a.id ,a.name from enum_const a where a.namespace='FlagPreTest'";
		columnConfigIsExam.setComboSQL(pretestsql);
		this.getGridConfig().addColumn(columnConfigPreTest);		
		...
		if (us.getUserLoginType().equals("3")) {
			...
			if (capabilitySet.contains(this.servletPath + "_view.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课程信息"),
								"{return '<a href=\"/entity/teaching/peBzzCoursePubManager_courseInfo.action?id='+${value}+'&indexFile='+record.data['indexFile']+'\"  target=\"_blank\">课程信息</a>';}",
								"id");
			}
			...
			if (capabilitySet.contains(this.servletPath + "_setTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("设置讲师图片"), "/entity/teaching/peBzzCoursePubManager_setTeacherImg.action", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_removeTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("删除讲师图片"), "/entity/teaching/peBzzCoursePubManager_removeImg.action?param=t", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_setSuggest.action")) {
				this.getGridConfig().addMenuFunction(this.getText("设置建议学习人群"), "/entity/teaching/peBzzCoursePubManager_setSuggest.action?table_name=PE_BZZ_TCH_COURSE", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_removeSuggest.action")) {
				this.getGridConfig().addMenuFunction(this.getText("删除建议学习人群"), "/entity/teaching/peBzzCoursePubManager_removeSuggest.action", false, false);
			}
			...
			if (capabilitySet.contains(this.servletPath + "_impCourse.action") && capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课件文件管理"),
								"{if(record.data['coursewareId'] == ''){return '&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/entity/teaching/peBzzCoursePubManager_toUploadCourseware.action?bean.id='+record.data['id']+'\" target=\"_blank\">导入</a>';} else return '<a href=\"#\" onclick=if(true){window.open(\"/entity/manager/course/courseware/filemanager/filemanager_main02.jsp?courseware_id='+record.data['code']+'\")} >管理</a>&nbsp;&nbsp;<a href=\"#\" onclick=if(confirm(\"确定要删除吗\")){window.open(\"/entity/manager/course/courseware/scorm12/delete_scorm_info.jsp?courseware_id='+record.data['code']+'\")} >删除</a>'}",
								"id");
			}
			if (capabilitySet.contains(this.servletPath + "_impCourse.action") && !capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课件文件管理"),
								"{if(record.data['coursewareId'] == ''){return '&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/entity/teaching/peBzzCoursePubManager_toUploadCourseware.action?bean.id='+record.data['id']+'\" target=\"_blank\">导入</a>';} else return '<a href=\"#\" onclick=if(confirm(\"确定要删除吗\")){window.open(\"/entity/manager/course/courseware/scorm12/delete_scorm_info.jsp?courseware_id='+record.data['code']+'\")} >删除</a>'}",
								"id");
			}
			if (!capabilitySet.contains(this.servletPath + "_impCourse.action") && capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this.getGridConfig()
			...
			if (capabilitySet.contains(this.servletPath + "_satified.action")) {
				this.getGridConfig().addRenderFunction(this.getText("满意度调查"),
						"<center><a href=\"/entity/teaching/addSatisfactionPaper.action?courseId=${value}&flag=addPaper&area=peBzzCoursePubManager\" >添加</a></center>", "id");
			}
	...
	public void setServletPath() {
		// TODO Auto-generated method stub
		this.servletPath = "/entity/teaching/peBzzCoursePubManager";
	}
...
//		this.getGeneralService().save(peBzzTchCourse);
		...
		/* 课程所属区域-公共区课程 */
		EnumConst ecFlagCoursearea = this.getEnumConstService().getByNamespaceCode("FlagCoursearea", "1");
		this.getBean().setEnumConstByFlagCoursearea(ecFlagCoursearea);
		/* 是否可以添加到专项中-是 */
		EnumConst ecByFlagCanJoinBatch = this.getEnumConstService().getByNamespaceCode("FlagCanJoinBatch", "1");
		this.getBean().setEnumConstByFlagCanJoinBatch(ecByFlagCanJoinBatch);
		/* 推荐状态-否 */
	...
	public void checkBeforeAdd() throws EntityException {
		...
		String exam = this.getBean().getEnumConstByFlagIsExam().getName();
		
		String pretest = this.getBean().getEnumConstByFlagPreTest().getName();
		
		if("否".equals(exam) && "是".equals(pretest)){
			throw new EntityException("不考试课程不能设置课前测试");
		}
		// 暂时不用名称限制
	...
	public Page list() {
		...
		try {
			...
			sqlBuffer.append(" EC4.NAME FLAGCONTENTPROPERTY, EC8.NAME FLAGISVALID, EC9.NAME FLAGOFFLINE, EC5.NAME FLAGISEXAM, ");// EC.NAME
																																	// FLAGISRECOMMEND,
																																	// lzh
			sqlBuffer.append(" EC11.NAME FlagPreTest,");
			sqlBuffer.append(" EC6.NAME FLAGISFREE, A.PRICE, A.TIME,  A.COURSE_LEN ,A.TEACHER, ");
			...
			// bug 440去掉to_char zgl 2014-10-21
			sqlBuffer.append(" A.CREATE_DATE CREATEDATE, WM_CONCAT(EC10.NAME) FS ");
			...
			sqlBuffer.append(" LEFT JOIN ENUM_CONST EC11 ON A.FLAG_PRE_TEST = EC11.ID ");
			...
			sqlBuffer.append(" WHERE A.FLAG_COURSEAREA = 'Coursearea1'");
			...
			sqlBuffer.append(" A.PRICE,A.TIME,A.COURSE_LEN ,A.TEACHER,A.STUDYDATES,A.END_DATE,A.ANNOUNCE_DATE, ");
			...
			sqlBuffer.append(" C.SCORM_TYPE, C.INDEXFILE, SATISFACTION_ID,TABLE_NAME,EC11.NAME ");
			...
			do {
				...
				//课前测试
				if(name.equals("enumConstByFlagPreTest.name")){
					name = "FlagPreTest";
				}
			...
			if (this.getSort() != null && temp != null) {
				...
				//课前测试
				if(temp.equals("enumConstByFlagPreTest.name")){
					temp = "FlagPreTest";
				}				
				if (temp.equals("id")) {
					temp = "createDate";
