...
public class OracleExamPaperHistory extends ExamPaperHistory {

	public OracleExamPaperHistory() {
	}

	public OracleExamPaperHistory(String id) {
		String sql = "select id, user_id,testpaper_id, to_char(test_date,'yyyy-mm-dd') as test_date,test_result,score from test_exampaper_history "
				+ "where id='" + id + "'";
	...
	public int add() throws PlatformException {
		dbpool db = new dbpool();
		String delSql = "delete from test_exampaper_history where user_id='"
				+ this.getUserId() + "' and testpaper_id='"
				+ this.getTestPaperId() + "'";
		db.executeUpdate(delSql);
		UserDeleteLog.setDebug("OracleExamPaperHistory.add() SQL=" + delSql +  " DATE=" + new Date());
		String sql = "insert into test_exampaper_history (id, user_id,testpaper_id, test_date, test_result,score) values ("
		...
		UserAddLog.setDebug("OracleExamPaperHistory.add() SQL=" + sql + " COUNT=" + suc + " DATE=" + new Date());
		String gradeSql = "update entity_elective	set exam_score ='"
				+ this.getScore()
				+ "' where student_id  ='"
				+ this.getUserId().substring(1, this.getUserId().indexOf(")"))
				+ "' and teachclass_id in (select group_id from test_exampaper_info where id ='"
				+ this.getTestPaperId() + "')";
		db.executeUpdate(gradeSql);
		UserUpdateLog.setDebug("OracleExamPaperHistory.add() SQL=" + sql + " DATE=" + new Date());
		return suc;
	}

	public int update() throws PlatformException {
		dbpool db = new dbpool();
		String sql = "update test_exampaper_history set testpaper_id='"
				+ this.getTestPaperId()
				+ "', test_date=sysdate, test_result=?, score='"
		...
		UserUpdateLog.setDebug("OracleExamPaperHistory.update() SQL=" + sql + " COUNT=" + i + " DATE=" + new Date());
		return i;
	}

	public int delete() throws PlatformException {
		dbpool db = new dbpool();
		String sql = "delete from test_exampaper_history where id='"
				+ this.getId() + "'";
		int i = db.executeUpdate(sql);
		UserDeleteLog.setDebug("OracleExamPaperHistory.delete() SQL=" + sql + " COUNT=" + i + " DATE=" + new Date());
		return i;
	}
	...
	public HashMap getTestResultMap() {
		...
		try { 
	//		System.out.println("this.getTestResult()->"+this.getTestResult());
