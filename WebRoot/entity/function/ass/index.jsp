...
function toTest(){
	if(confirm('您已经完成本课程的课件学习，现在可以进行本课程的课后测验了，\n\n必须通过课后测验才能够获取学时，是否现在进入？')){
		...
		var toUrl = '/entity/function/ass/onlinetestcourse_list.jsp?opencourseId=<%=opencourseId%>';
...
function openCourseWare(){
	...
	windows['courseWin'] = window.open('<%=path%>/ass_training/student/course/jumpCourseware.jsp?courseId=<%=courseId%>&opencourseId=<%=opencourseId%>', 'courseWin', 'height=680,width=1050,top=0,left=50,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
}
											 ...
											 %>
												<tr>
													<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
														<a href="/entity/function/ass/course_state.jsp?courseId=<%=courseId%>&exam_y_n=<%=exam_y_n %>&courseCode=<%=openCourse.getBzzCourse().getCode() %>&opencourseId=<%=opencourseId %>" target="mainer" class="menuzi">课程状态</a>
													</td>
												</tr>
												<tr>
													<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
												...
												<%
													//已经学完并且课程有满意度ID
													if (peBzzAssService.isLearningCompleted(opencourseId, userId) && voteId != null && !"".equals(voteId)) {
														//是否存在成绩，如果成绩为空则显示满意度，如果不为空则可直接进入考试
														if (peBzzAssService.isShowManyi(userId, openCourse.getId())) {
															//课程需要考试
															if (openCourse.getBzzCourse().getEnumConstByFlagIsExam() != null && "1".equalsIgnoreCase(openCourse.getBzzCourse().getEnumConstByFlagIsExam().getCode())) {
																...
																Map scoreM = peBzzAssService.getOnlineTestScore(opencourseId, userId);
												...
												<%
															...
															//没有填写过满意度
															if (!peBzzAssService.isManyiCompleted(openCourse.getBzzCourse().getId(), userId)) {
												%>
												<tr>
													<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
														<a href="/entity/function/ass/manyidu_list.jsp?opencourseId=<%=opencourseId%>" target="mainer" class="menuzi">满意度调查</a>
													</td>
												</tr>
												<script>window.onload=function (){toManyi();}</script>
												<%
													//填写过满意度并且课程需要考试
													} else if (peBzzAssService.isManyiCompleted(openCourse.getBzzCourse().getId(), userId) && openCourse.getBzzCourse().getEnumConstByFlagIsExam() != null
												...
												<%
													...
													<tr>
														<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
															<a href="/entity/function/ass/manyidu_list.jsp?opencourseId=<%=opencourseId%>&flag=false" target="mainer" class="menuzi">满意度调查</a>
														</td>
													</tr><%
															}
														}
												...
												<%
													...
													<tr>
														<td width="138" height="45" align="center" valign="top" background="images/menu1_1bg.jpg" style="background-repeat: no-repeat; padding-top: 13px; padding-left: 33px; background-position: center">
															<a href="/entity/function/ass/onlinetestcourse_list.jsp?opencourseId=<%=opencourseId%>&flag=<%=exam_flag %>" target="mainer" class="menuzi">课后测验</a>
														</td>
													</tr>
													<%
													}
