...
try {
            ...
            List<MsgInfo> list = (List<MsgInfo>) this.getHibernateTemplate().find(LOAD_BY_ID,id);
	...
	public boolean getCurStatus(String userId) {
		 List<MsgInfo> list = (List<MsgInfo>) this.getHibernateTemplate().find(UNREADED_IDS,userId);
         if(list ==null || list.isEmpty()){
 			return false;
 		}else{
 			return true;
