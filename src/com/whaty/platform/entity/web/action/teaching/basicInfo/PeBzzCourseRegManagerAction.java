...
public class PeBzzCourseRegManagerAction extends MyBaseAction<PeBzzTchCourse> {
	...
	public void initGrid() {
		...
		this.getGridConfig().setTitle("监管机构课程发布");
		...
		// ColumnConfig columnConfigType = new
		// ColumnConfig(this.getText("课程性质"), "enumConstByFlagCourseType.name",
		// true, true, false, "TextField", false, 100, "");
		// String sql = "select a.id ,a.name from enum_const a where
		// a.namespace='FlagCourseType'";
		// columnConfigType.setComboSQL(sql);
		// this.getGridConfig().addColumn(columnConfigType);
		//
		// ColumnConfig columnConfigCategory = new
		// ColumnConfig(this.getText("业务分类"),
		// "enumConstByFlagCourseCategory.name", true, true, false, "TextField",
		// false, 100, "");
		// String sql2 = "select a.id ,a.name from enum_const a where
		// a.namespace='FlagCourseCategory'";
		// columnConfigCategory.setComboSQL(sql2);
		// this.getGridConfig().addColumn(columnConfigCategory);
		//
		// ColumnConfig columnCourseItemType = new
		// ColumnConfig(this.getText("大纲分类"),
		// "enumConstByFlagCourseItemType.name", true, true, false, "TextField",
		// false, 100, "");
		// String sql7 = "select a.id ,a.name from enum_const a where
		// a.namespace='FlagCourseItemType'";
		// columnCourseItemType.setComboSQL(sql7);
		// this.getGridConfig().addColumn(columnCourseItemType);
		//
		// ColumnConfig columnContentProperty = new
		// ColumnConfig(this.getText("内容属性分类"),
		// "enumConstByFlagContentProperty.name", true, true, false,
		// "TextField", false, 100, "");
		// String sql9 = "select a.id ,a.name from enum_const a where
		// a.namespace='FlagContentProperty'";
		...
		// this.getGridConfig().addColumn(this.getText("金额"), "price", true,
		// true, true, Const.fee_for_extjs);

		this.getGridConfig().addColumn(this.getText("课程时长(分钟)"), "courseLen", false, true, true, "TextField", true, 25);
		...
		// ColumnConfig ccFlagSuggest = new ColumnConfig(this.getText("建议学习人群"),
		// "enumConstByFlagSuggest.name", true, false, false, "TextField",
		// false, 100, "");
		// String sqlFlagSuggest = "select a.id ,a.name from enum_const a where
		// a.namespace='FlagSuggest' and name not in
		// ('面向所有人投票','面向学员投票','面向集体管理员投票')";
		...
		if (us.getUserLoginType().equals("3")) {
			...
			if (capabilitySet.contains(this.servletPath + "_view.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课程信息"),
								"{return '<a href=\"/entity/teaching/peBzzCourseRegManager_courseInfo.action?id='+${value}+'&indexFile='+record.data['indexFile']+'\"  target=\"_blank\">课程信息</a>';}",
								"id");
			}
			...
			if (capabilitySet.contains(this.servletPath + "_applyPub.action")) {
				this.getGridConfig().addMenuFunction(this.getText("申请发布"), "checkStatus");
			}
			if (capabilitySet.contains(this.servletPath + "_agreePub.action")) {
				this.getGridConfig().addMenuFunction(this.getText("同意发布"), "FlagIsvalid.true");
			}
			if (capabilitySet.contains(this.servletPath + "_refusePub.action")) {
			...
			if (capabilitySet.contains(this.servletPath + "_setTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("设置讲师图片"), "/entity/teaching/peBzzCourseRegManager_setTeacherImg.action", false, false);
			}
			if (capabilitySet.contains(this.servletPath + "_removeTeaImg.action")) {
				this.getGridConfig().addMenuFunction(this.getText("删除讲师图片"), "/entity/teaching/peBzzCourseRegManager_removeImg.action?param=t", false, false);
			}
			// if (capabilitySet.contains(this.servletPath +
			// "_setSuggest.action")) {
			// this.getGridConfig().addMenuFunction(this.getText("设置建议学习人群"),
			// "/entity/teaching/peBzzCoursePubManager_setSuggest.action",
			// false, false);
			// }
			// if (capabilitySet.contains(this.servletPath +
			// "_removeSuggest.action")) {
			// this.getGridConfig().addMenuFunction(this.getText("删除建议学习人群"),
			// "/entity/teaching/peBzzCoursePubManager_removeSuggest.action",
			// false, false);
			// }
			...
			if (capabilitySet.contains(this.servletPath + "_impCourse.action") && capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课件文件管理"),
								"{if(record.data['coursewareId'] == ''){return '&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/entity/teaching/peBzzCourseRegManager_toUploadCourseware.action?bean.id='+record.data['id']+'\" target=\"_blank\">导入</a>';} else return '<a href=\"#\" onclick=if(true){window.open(\"/entity/manager/course/courseware/filemanager/filemanager_main02.jsp?courseware_id='+record.data['code']+'\")} >管理</a>&nbsp;&nbsp;<a href=\"#\" onclick=if(confirm(\"确定要删除吗\")){window.open(\"/entity/manager/course/courseware/scorm12/delete_scorm_info.jsp?courseware_id='+record.data['code']+'\")} >删除</a>'}",
								"id");
			}
			if (capabilitySet.contains(this.servletPath + "_impCourse.action") && !capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this.getGridConfig()
						.addRenderScript(
								this.getText("课件文件管理"),
								"{if(record.data['coursewareId'] == ''){return '&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/entity/teaching/peBzzCourseRegManager_toUploadCourseware.action?bean.id='+record.data['id']+'\" target=\"_blank\">导入</a>';} else return '<a href=\"#\" onclick=if(confirm(\"确定要删除吗\")){window.open(\"/entity/manager/course/courseware/scorm12/delete_scorm_info.jsp?courseware_id='+record.data['code']+'\")} >删除</a>'}",
								"id");
			}
			if (!capabilitySet.contains(this.servletPath + "_impCourse.action") && capabilitySet.contains(this.servletPath + "_manageCourse.action")) {
				this.getGridConfig()
			...
			if (capabilitySet.contains(this.servletPath + "_satified.action")) {
				this.getGridConfig().addRenderFunction(this.getText("满意度调查"),
						"<center><a href=\"/entity/teaching/addSatisfactionPaper.action?courseId=${value}&flag=addPaper&area=peBzzCourseRegManager\" >添加</a></center>", "id");
			}
	...
	public void setServletPath() {
		// TODO Auto-generated method stub
		this.servletPath = "/entity/teaching/peBzzCourseRegManager";
	}
	...
	public Map add() {
		...
		/* 建议学习人群 */
		// EnumConst ecFlagSuggestSet =
		// this.getEnumConstService().getByNamespaceCode("FlagFlagSuggestSet",
		// "0");
		...
		/* 课程所属区域-公共区课程 */
		EnumConst ecFlagCoursearea = this.getEnumConstService().getByNamespaceCode("FlagCoursearea", "5");
		this.getBean().setEnumConstByFlagCoursearea(ecFlagCoursearea);
		/* 课程收费状态-免费 */
		EnumConst ecFlagIsFree = this.getEnumConstService().getByNamespaceCode("FlagIsFree", "1");
		this.getBean().setEnumConstByFlagIsFree(ecFlagIsFree);
		// 李文强 2014-10-13 协会内训课程默认学时数为0
		this.getBean().setTime("0");
		/* 创建日期 */
	...
	public int training() {
		...
		if (this.getIds() != null && this.getIds().length() > 0) {
			...
			if (str != null && str.length() > 0) {
				...
				String sql = "SELECT COUNT(ID)  FROM PE_BZZ_BATCH WHERE ID IN (SELECT FK_BATCH_ID FROM PR_BZZ_TCH_OPENCOURSE " + "WHERE FK_COURSE_ID IN (" + idss + ")) "
						+ "and FLAG_BATCH_TYPE IS NOT NULL AND FLAG_BATCH_TYPE != '40288a7b394d676d01394db3990d003e' AND END_TIME > SYSDATE";
				try {
	...
	public void checkBeforeAdd() throws EntityException {
		// String flagIsFree =
		// this.getBean().getEnumConstByFlagIsFree().getName();
	...
	public Page list() {
		...
		try {
			sqlBuffer.append("SELECT * FROM (SELECT A.ID, A.CODE, A.NAME,  ");
			sqlBuffer.append("  EC8.NAME FLAGISVALID, EC9.NAME FLAGOFFLINE, EC5.NAME FLAGISEXAM,  ");
			sqlBuffer.append("  A.COURSE_LEN AS courseLen, A.TEACHER, ");
			...
			// sqlBuffer.append(" INNER JOIN ENUM_CONST EC1 ON A.FLAG_COURSETYPE
			// = EC1.ID ");
			// sqlBuffer.append(" INNER JOIN ENUM_CONST EC2 ON
			// A.FLAG_COURSECATEGORY = EC2.ID ");
			// sqlBuffer.append(" INNER JOIN ENUM_CONST EC3 ON
			// A.FLAG_COURSE_ITEM_TYPE = EC3.ID ");
			// sqlBuffer.append(" INNER JOIN ENUM_CONST EC4 ON
			// A.FLAG_CONTENT_PROPERTY = EC4.ID ");
			sqlBuffer.append(" INNER JOIN ENUM_CONST EC5 ON A.FLAG_IS_EXAM = EC5.ID ");
			// sqlBuffer.append(" INNER JOIN ENUM_CONST EC ON A.FLAG_ISRECOMMEND
			// = EC.ID");
			// sqlBuffer.append(" INNER JOIN ENUM_CONST EC6 ON A.FLAG_ISFREE =
			// EC6.ID ");
			...
			sqlBuffer.append(" WHERE A.FLAG_COURSEAREA = 'Coursearea5'");
			sqlBuffer.append(" GROUP BY A.ID,A.CODE,A.NAME, ");
			sqlBuffer.append(" EC8.NAME,EC9.NAME,EC5.NAME, ");
			// sqlBuffer.append(" EC8.NAME,EC9.NAME,EC5.NAME, ");
			sqlBuffer.append(" A.COURSE_LEN,A.TEACHER,A.STUDYDATES,A.END_DATE,A.ANNOUNCE_DATE, ");
			...
			if (this.getSort() != null && temp != null) {
				...
				if (this.getDir() != null && this.getDir().equalsIgnoreCase("desc")) {
					if ("time".equalsIgnoreCase(temp) || "price".equalsIgnoreCase(temp)||"courseLen".equalsIgnoreCase(temp)) {
			...
			try {
				...
				if (count != 0) {
					...
					if (action.equals("checkStatus")) {
						...
						while (iterator.hasNext()) {
							PeBzzTchCourse peBzzTchCourse = iterator.next();
							// if(!"0".equals(peBzzTchCourse.getEnumConstByFlagIsvalid().getCode()))
							// {
							// map.clear();
							// map.put("success", "false");
							// map.put("info", "课程处于未发布状态才能申请发布");
							// return map;
							// }
							String logs = peBzzTchCourse.getOperationLogs();
							if (logs == null) {
	...
	public String courseInfo() {
		...
		try {
			...
			sb.append("SELECT A.CODE,A.NAME,A.TEACHER,A.STUDYDATES,A.END_DATE, ");
			sb.append(" A.ANSWER_BEGINDATE,A.ANSWER_ENDDATE,A.COURSE_LEN,A.PASSROLE,A.EXAMTIMES_ALLOW, ");
			sb.append(" EC2.NAME FLAGISVISITAFTERPASS,EC3.NAME FLAGISEXAM, ");
			// sb.append(" LISTAGG(C.NAME, ',') WITHIN GROUP(ORDER BY C.NAME)
			// WC, ");
			sb.append(" A.PHOTO_LINK,A.TEA_IMG,A.PASSROLE_NOTE,A.SUGGESTION,ssl.indexfile ");
			sb.append(" FROM (SELECT * FROM PE_BZZ_TCH_COURSE WHERE ID = '" + id + "') A ");
			// sb.append(" INNER JOIN ENUM_CONST EC1 ON A.FLAG_COURSETYPE =
			// EC1.ID ");
			...
			// sb.append(" INNER JOIN ENUM_CONST EC4 ON A.FLAG_COURSECATEGORY =
			// EC4.ID ");
			// sb.append(" INNER JOIN ENUM_CONST EC5 ON A.FLAG_COURSE_ITEM_TYPE
			// = EC5.ID ");
			// sb.append(" INNER JOIN ENUM_CONST EC6 ON A.FLAG_CONTENT_PROPERTY
			// = EC6.ID ");
			// sb.append(" INNER JOIN ENUM_CONST EC7 ON A.FLAG_ISRECOMMEND =
			// EC7.ID ");
			// sb.append(" INNER JOIN ENUM_CONST EC8 ON A.FLAG_ISFREE = EC8.ID
			// ");
			// sb.append(" LEFT JOIN (SELECT * FROM PE_BZZ_TCH_COURSE_SUGGEST
			// WHERE TABLE_NAME = 'PE_BZZ_TCH_COURSE') B ");
			...
			sb.append(" GROUP BY A.CODE,A.NAME,A.TEACHER,A.STUDYDATES,A.END_DATE, ");
			sb.append(" A.ANSWER_BEGINDATE,A.ANSWER_ENDDATE,A.COURSE_LEN,A.PASSROLE, ");
			sb.append(" A.EXAMTIMES_ALLOW,EC2.NAME,EC3.NAME, ");
			sb.append(" A.PHOTO_LINK,A.TEA_IMG,A.PASSROLE_NOTE,A.SUGGESTION,ssl.indexfile ");
			...
			// this.indexFile =
			// ServletActionContext.getRequest().getParameter("indexFile");
