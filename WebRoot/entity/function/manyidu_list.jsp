...
.td {
	...
	<%
		...
		String sql = " SELECT DISTINCT B.ID, A.TITLE, NVL(TO_CHAR(C.VOTE_DATE,'yyyy-MM-dd'),'-') VOTE_DATE FROM PE_VOTE_PAPER A INNER JOIN (SELECT ID, FK_PARENT_ID FROM PE_VOTE_PAPER "
				+ " WHERE COURSEID = (SELECT FK_COURSE_ID FROM PR_BZZ_TCH_OPENCOURSE WHERE ID = '" + opencourseId + "')) B "
				+ " ON A.ID = B.FK_PARENT_ID LEFT JOIN (SELECT * FROM PR_VOTE_RECORD WHERE SSOID = '" + user.getId() + "') C ON B.ID = C.FK_VOTE_PAPER_ID ";//Lee 2014年9月18日14:37:02 去掉满意度为有效的条件 WHERE A.FLAG_ISVALID = '2'
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<tr>
								...
								<td class="td">
									...
									%>
										<a href="###" onClick="window.parent.open('/entity/first/firstPeVotePaper_toVote.action?bean.id=<%=mydList.get(0)%>&togo=1&opencourseId=<%=opencourseId%>')" class="menuzi">填写</a>
									<%
										} else {
									%>
