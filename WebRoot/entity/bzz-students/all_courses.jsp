...
function resetContent(){
	...
	<body>
		<form action="/entity/workspaceStudent/studentWorkspace_toAllCourses.action" name="frm" method="post">
						...
						<tr>
							...
							<td width="13%" align="left">
								<select name="orderType" class="sub">
									...
									<option selected value="starttime" >
										课程编号
									</option>
									<option selected value="lasttime" >
										课程编号
									</option>
								</select>
							</td>
						...
						<s:if test="page != null">
							<s:iterator value="page.items" id="electiveCourse" status="item">
								...
								<tr>
									...
									<td>
										<a style="color: #3366ff; cursor: pointer;" onclick="window.open('/cms/flkcalone.htm?myId=<s:property value="#electiveCourse[2]"/>','newwindow_detail')"'><s:property value="#electiveCourse[4]" /> </a>
										<!-- 查看页面没做完
									...
									<td>
										...
										<s:if test="#swa.CompareTime(#sd,#ss)">
											<s:if test="#electiveCourse[12]=='INCOMPLETE'">正在学习</s:if>
											<s:elseif test="#electiveCourse[12]=='COMPLETED' && #electiveCourse[17] != 1 && #electiveCourse[16] == 0">待考试</s:elseif>
											<s:elseif test="#electiveCourse[17] == 1">已完成</s:elseif>
											<s:else>未开始学习</s:else>
										</s:if>
										<s:else>
											<s:if test="#electiveCourse[13]==1">
											...
											<s:elseif test="#electiveCourse[17]==1">
												已完成
											</s:elseif>
											<s:else>
									    		已过期
								   		...
								   		-->
								   		<s:property value="#electiveCourse[12]"/>
									</td>
									<td>
										...
										<s:else>
												...
												<s:elseif test="#electiveCourse[12] != '已过期'">
													<div class="ck" style="width: 70px; background-image: url('/entity/bzz-students/images/index_22.jpg')">
												...
												<s:elseif test="#electiveCourse[12] == '已过期'">
													<div class="ck" style="width: 70px; background-image: url('/entity/bzz-students/images/index_22.jpg')">
														<a style="cursor: pointer;" onclick="toRePayment('<s:property value="#electiveCourse[1]"/>');">重新购买</a><br/>
													</div>
													<div class="ck" style="width: 70px; background-image: url('/entity/bzz-students/images/index_22.jpg')">
														<a style="cursor: pointer;" href="/entity/workspaceStudent/studentWorkspace_courseInfo.action?courseId=<s:property value="#electiveCourse[2]"/>&opencourseId=<s:property value="#electiveCourse[1]"/>&sid=<s:property value="#request.sid" />&courseCode=<s:property value="#electiveCourse[3]"/>" target=_blank>学习记录</a>
													</div>
												</s:elseif>
										</s:else>
									</td>
