...
public class OracleReExamBatch extends ReExamBatch {
	public OracleReExamBatch() {
	}
	
	public OracleReExamBatch(String aid) {
		dbpool db = new dbpool();
		String sql = "";
		MyResultSet rs = null;
		sql = "select name,to_char(s_date,'yyyy-mm-dd') as s_date,to_char(e_date,'yyyy-mm-dd') as e_date,status,to_char(examroom_s_date,'yyyy-mm-dd') as examroom_s_date,to_char(examroom_e_date,'yyyy-mm-dd') as examroom_e_date from expendtest_exambatch_info where id='"
				+ aid + "'";
		try {
	...
	public int add() throws PlatformException {
		...
		String sql1 = "select * from expendtest_exambatch_info where id="+this.getId();
		...
		String sql = "insert into expendtest_exambatch_info (id,name,s_date,e_date,examroom_s_date,examroom_e_date,status) values ('"
				+ this.getId()
				+ "','"
				+ this.getName()
				+ "',to_date('"
	...
	public int update() throws PlatformException {
		dbpool db = new dbpool();
		String sql = "";
		sql = "update expendtest_exambatch_info set name='" + this.getName()
 ...
 */
	public int delete() throws PlatformException {
		...
		String sql1 = "select t.id from expendtest_examcourse_info t where t.test_batch_id='"+this.getId()+"'";
		...
		String sql = "";
		sql = "delete from expendtest_exambatch_info where id='" + this.getId() + "'";
		int i = db.executeUpdate(sql);
