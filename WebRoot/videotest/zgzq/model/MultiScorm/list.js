...
function Collapse1()
{
	for (i=1;i<=xmlDoc.selectNodes("courseslides/slide").length;i++) {
		...
		key=document.getElementById("xitem" + i);
		if (key)
			key.innerHTML=getCloseICON(i);
	}
}

function getVisibleNode(strItemNo)
{
	if(strItemNo.toString().indexOf("_") != -1)
		return strItemNo;
	var signItemHidden = IsNodeVisible(xmlDoc.selectNodes("courseslides/slide").item(strItemNo-1));//.selectSingleNode("hidden").nodeTypedValue;
	if(signItemHidden=="true")
	{
		//
		var tnum=strItemNo;
		var cnum=0;
		for(var ii=(strItemNo-1);ii>=0;ii--)
		{
			if(IsNodeVisible(xmlDoc.selectNodes("courseslides/slide").item(ii))=="false")
			{
				return (ii+1);
			}
			else
		...
		if(tnum==cnum)
		{
			...
			{
				if(IsNodeVisible(xmlDoc.selectNodes("courseslides/slide").item(i))=="false")
				{
					return (i+1);
				}
			}
...
function getNextVisibleNode(strItemNo)
{
	for(var i=(strItemNo);i<xmlDoc.selectNodes("courseslides/slide").length;i++)
	{
		if(IsNodeVisible(xmlDoc.selectNodes("courseslides/slide").item(i))=="false")
		{
			return (i+1);
...
function SelectLink(strItemNo)
{
  var pListItem, pListItem1;
	...
	var xmlSlide;
	var signItemNo = getVisibleNode(strItemNo);
    ...
    */
    ...
    pListItem = document.getElementById("ListItem_" + signItemNo);
    if (pListItem)
    {
...
{
	if(node.selectSingleNode("hidden")==null)
	{
		return "false";
	}
	else
	{
		return node.selectSingleNode("hidden").nodeTypedValue.toLowerCase()
	}
}
