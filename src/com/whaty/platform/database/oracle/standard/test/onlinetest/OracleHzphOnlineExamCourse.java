...
public class OracleHzphOnlineExamCourse extends OnlineTestCourse {
	public OracleHzphOnlineExamCourse() {
	};
	
	public OracleHzphOnlineExamCourse(String id) {
		String sql = "select id,title,group_id,batch_id,note,start_date,end_date,isautocheck,ishiddenanswer,status,creatuser,creatdate,fk_batch_id "
				...
				+ " from hzph_Exam_COURSE_INFO) where id='" + id + "'";
	...
	public int add() throws PlatformException {
		dbpool db = new dbpool();
		String sql = "insert into hzph_Exam_COURSE_INFO (id,title,group_id,batch_id,note,start_date,end_date,isautocheck,ishiddenanswer,status,creatuser,creatdate) values ("
				+ "to_char(HZPHONLINEEXAM_COURSE_INFO_ID.nextval), '"
				+ this.getTitle()
				+ "', '"
				+ this.getGroupId()
				+ "', '"
	...
	public int add(boolean flag) throws PlatformException {
		dbpool db = new dbpool();
		String sql = "insert into hzph_Exam_COURSE_INFO (id,title,group_id,batch_id,note,start_date,end_date,isautocheck,ishiddenanswer,status,creatuser,creatdate,fk_batch_id ) values ("
				+ "to_char(HZPHONLINEEXAM_COURSE_INFO_ID.nextval), '"
				+ this.getTitle()
				+ "', '"
				+ this.getGroupId()
...
//				+this.getBatch_id()+"')";
	...
	public int update() throws PlatformException {
		dbpool db = new dbpool();
		String sql = "update hzph_Exam_COURSE_INFO set title='"
	...
	public int delete() throws PlatformException {
		...
		String checkSql = 
			"select *\n" +
			"  from hzph_Exam_COURSE_INFO ocp, HZPH_EXAMPAPER_INFO tti\n" + 
		...
		String sql = "delete from hzph_Exam_COURSE_INFO where id='"
				+ this.getId() + "'";
		int i = db.executeUpdate(sql);
	...
	public int cancelActive() {
		dbpool db = new dbpool();
		String sql = "";
		sql = "update hzph_Exam_COURSE_INFO set status='0' where id ='"
				+ this.getId() + "'";
		int i = db.executeUpdate(sql);
	...
	public int setActive() {
		dbpool db = new dbpool();
		String sql = "";
		sql = "update hzph_Exam_COURSE_INFO set status='1' where id ='"
				+ this.getId() + "'";
		int i = db.executeUpdate(sql);
...
//	}
	...
	public int getTestPapersNum() {
		...
		String sql = "select id,title,creatuser,creatdate,status,note,time,type,group_id from "
				+ "(select a.id,a.title,a.creatuser,to_char(a.creatdate,'yyyy-mm-dd') as creatdate,a.status,a.note,"
				+ "a.time,a.type,a.group_id from HZPH_EXAMPAPER_INFO a,onlinetest_course_paper b where a.id=b.paper_id and b.testcourse_id='"
				+ this.getId() + "')";
