...
public class OracleReTestUser extends TestUser {

	public OracleReTestUser() {
	}

	public OracleReTestUser(String aid) {
		dbpool db = new dbpool();
		String sql = "";
		MyResultSet rs = null;
		sql = "select name,email,ftpaccount,login_id,login_type,password from expendtest_user_info where id='"
				+ aid + "'";
		try {
	...
	public int add() throws PlatformException {
		...
		int i = 1;
		if (IsExist() <= 0) {
			sql = "insert into expendtest_user_info (id,name,email,ftpaccount,login_id,login_type,password) values ('"
					+ this.getId()
					+ "','"
					+ this.getName()
					+ "','"
	...
	public int update() throws PlatformException {
		dbpool db = new dbpool();
		String sql = "";
		sql = "update expendtest_user_info set name='" + this.getName() + "',email='"
	...
	public int delete() throws PlatformException {
		dbpool db = new dbpool();
		String sql = "";
		sql = "delete from expendtest_user_info where id='" + this.getId() + "'";
		int i = db.executeUpdate(sql);
	...
	public int IsExist() throws PlatformException {
		dbpool db = new dbpool();
		String sql = "";
		sql = "select *  from expendtest_user_info where login_id='"
				+ this.getLoginId() + "'";
